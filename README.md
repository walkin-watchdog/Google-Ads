...
2. Verify fresh schema initialization, login, magic links, invite/reset email, logout, disabled-user rejection, and the installable PWA shell.
...
- `STARTUP_REFRESH`: enables a refresh after the HTTP server is healthy. It does not delay server availability; the normal refresh-queue worker starts after this isolated startup refresh exits so the two executors cannot compete. Keep it `false` on small VPS machines to avoid refresh load competing with dashboard traffic.
...
- `MCP_API_KEYS_JSON`: required when `NODE_ENV=production`; MCP access uses only scoped keys from this configuration.
...
7. Scroll down to **Variables and Secrets** and click **New secret** to add all required environment variables. You can use `backend/.env.hf.example` as the checklist:
   ...
   - *Add your Google Ads credentials. The backend accepts the shorter names below and the `GOOGLE_ADS_*` aliases used in `backend/.env.example`:*
     ...
     - Dashboard warehouse watermarks are maintained in `google_ads_warehouse_slice_fingerprints` during warehouse writes, so cached dashboard slices validate freshness with one fingerprint-table read instead of scanning every warehouse source table.
...
- Omit `HF_TOKEN` from MCP config when using the bundled proxy; `MCP/mcp-server.js` sends only `MCP_API_KEY` to `/api/mcp`.
...
Run `cd backend && bun run db:migrate` after configuring `DATABASE_URL`. The repository starts with a clean baseline migration for a new database; append new ordered migrations for future schema changes.
