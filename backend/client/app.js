...
const LEAD_PLATFORM_STORAGE_KEY = 'leadReviewAdPlatform';
...
function currentLeadReviewAttributionForExport() {
    const current = leadReviewAttributionForPlatform();
    return {
        ...current,
...
function leadReviewCsvFilename() {
    ...
    const scope = [
        activeLeadPlatform,
        activeLeadPlatform === 'google' && campaign !== 'All' ? campaign : '',
        activeLeadPlatform === 'google' && adGroup !== 'All' ? adGroup : ''
    ]
        .filter(Boolean)
        .join('-')
        .replace(/[^a-z0-9]+/gi, '-')
        .replace(/^-+|-+$/g, '')
        .toLowerCase();
    return `lead-review-all-time${scope ? `-${scope}` : ''}.csv`;
}
...
function updateLeadReviewCsvButton(leads = currentLeadReviewExportLeads()) {
    ...
    btn.title = count > 0
        ? `Export all-time ${activeLeadPlatform === 'meta' ? 'Meta' : 'Google'} lead review rows matching the current account filters.`
        : `No all-time ${activeLeadPlatform === 'meta' ? 'Meta' : 'Google'} lead review rows match the current account filters.`;
}
...
function setupLeadExportControls() {
    ...
    if (offlineBtn && !offlineBtn.hasAttribute('data-bound')) {
        offlineBtn.addEventListener('click', async () => {
            const readiness = leadReviewAttributionForPlatform(dashboardData?.leadAttribution, 'google')?.offlineExport || {};
    ...
    if (leadReviewBtn && !leadReviewBtn.hasAttribute('data-bound')) {
        leadReviewBtn.addEventListener('click', async () => {
            ...
            try {
                const params = new URLSearchParams();
                params.set('platform', activeLeadPlatform);
                if (activeLeadPlatform === 'google') {
                    const campaignId = currentSelectedCampaignId();
                    if (campaignId) params.set('campaignId', campaignId);
                ...
                if (rows === 0) {
                    showToast(`No ${activeLeadPlatform === 'meta' ? 'Meta' : 'Google'} lead review rows match the current filters.`, true);
                    return;
                }
                const blob = await res.blob();
                const url = URL.createObjectURL(blob);
...
function renderGlobalKPIs() {
    ...
    const metaLeadTotals = dashboardData.leadAttribution?.platforms?.meta?.totals || {};
    const metaLeadTotal = Number(metaLeadTotals.uniqueLeads || 0);
    const metaLeadMaybe = Number(metaLeadTotals.maybe || 0);
    const metaLeadQualified = Number(metaLeadTotals.qualified || 0);
    const metaLeadWon = Number(metaLeadTotals.converted || 0);
    ...
    if (mobileLeadsCard) {
        if (dashboardData.leadAttribution) {
            ...
            mobileLeadsCard.innerHTML = `
                ...
                <div class="card glass-card mobile-leads-card-inner mobile-leads-card-inner--meta">
                    <div class="mobile-leads-card-header">
                        <span class="mobile-leads-card-title">
                            <span class="mobile-leads-platform-mark mobile-leads-platform-mark--meta" aria-hidden="true">M</span>
                            Meta Leads
                        </span>
                        <span class="mobile-leads-card-value">${fmtNum(metaLeadTotal)}</span>
                    </div>
                    <div class="mobile-leads-card-badges">
                        <span class="mobile-leads-badge mobile-leads-badge--maybe"><strong>${fmtNum(metaLeadMaybe)}</strong> maybe</span>
                        <span class="mobile-leads-badge mobile-leads-badge--qualified"><strong>${fmtNum(metaLeadQualified)}</strong> qualified</span>
                        ${metaLeadWon > 0 ? `<span class="mobile-leads-badge mobile-leads-badge--won"><strong>${fmtNum(metaLeadWon)}</strong> won</span>` : ''}
                    </div>
                </div>
            `;
        } else {
    ...
    if (dashboardData.leadAttribution) {
        ...
        kpis.push({
            label: 'Meta Leads',
            value: fmtNum(metaLeadTotal),
            desc: `<strong>${metaLeadMaybe}</strong> maybe, <strong>${metaLeadQualified}</strong> qualified`,
            isLeads: true,
            leadPlatform: 'meta'
        });
    ...
    globalKpiGrid.innerHTML = kpis.map(kpi => {
        ...
        const leadsAttribute = kpi.isLeads
            ? `data-kpi-type="leads" data-lead-platform="${esc(kpi.leadPlatform || 'google')}"`
            : '';
...
function buildOverviewTimeSeriesRows(data, metricKeys = []) {
    const dailyRows = Array.isArray(data?.dailyTrend) ? data.dailyTrend : [];
    const range = data?.meta?.dateRange || {};
    const isSingleDay = Boolean(range.start) && range.start === range.end;
    const hourSource = Array.isArray(data?.dayAndHourPerformance) ? data.dayAndHourPerformance : [];
    const hourlyMetricKeys = new Set(['conversions', 'cpa', 'cvr', 'spend', 'impressions', 'clicks', 'ctr', 'avgCpc']);
    const hasUnsupportedMetric = metricKeys.some(metricKey => !hourlyMetricKeys.has(metricKey));
    if (!isSingleDay || hourSource.length === 0 || hasUnsupportedMetric) {
        return { rows: dailyRows, isHourly: false };
    }

    const byHour = new Map();
    hourSource.forEach(row => {
        ...
        bucket.conversionsValue += Number(row.conversionsValue || 0);
        bucket.allConversions += Number(row.allConversions || 0);
    });

    const rows = Array.from({ length: 24 }, (_unused, hour) => {
        const bucket = byHour.get(hour) || {
            hour,
            spend: 0,
            clicks: 0,
        ...
        return {
            ...bucket,
            ctr: bucket.impressions ? (bucket.clicks / bucket.impressions) * 100 : 0,
            avgCpc: bucket.clicks ? bucket.spend / bucket.clicks : 0,
            cpa: bucket.conversions ? bucket.spend / bucket.conversions : 0,
            cvr: bucket.clicks ? (bucket.conversions / bucket.clicks) * 100 : 0
        };
    });
    return { rows, isHourly: true };
}

function formatOverviewHour(hour) {
...
function formatOverviewHourRange(hour) {
    const startHour = Number(hour);
    if (!Number.isInteger(startHour) || startHour < 0 || startHour > 23) return '';
    return `${formatOverviewHour(startHour)} – ${formatOverviewHour((startHour + 1) % 24)}`;
}
...
function renderOverviewTimeSeries() {
    ...
    const visibleMetricKeys = overviewTimeSeriesCardMetrics
        .filter((_metricKey, cardSlot) => overviewTimeSeriesVisibleSlots[cardSlot]);
    const { rows: trendRows, isHourly } = buildOverviewTimeSeriesRows(dashboardData, visibleMetricKeys);
    ...
    const hasData = trendRows.length > 0 && visibleMetricCount > 0;
    const selectedMetricLabels = visibleMetricKeys
        .map(metricKey => OVERVIEW_TIME_SERIES_METRICS[metricKey].label.toLowerCase());
    canvas.setAttribute(
        'aria-label',
        `${selectedMetricLabels.join(', ')} ${isHourly ? 'by hour' : 'over the selected date range'}`
    );
    ...
    const labels = trendRows.map(row => isHourly ? formatOverviewHour(row.hour) : formatIsoDateShort(row.date));
    const datasets = overviewTimeSeriesCardMetrics
        .flatMap((metricKey, cardSlot) => {
            ...
            const rawValues = trendRows.map(row => overviewTimeSeriesMetricValue(metricKey, row));
            return [{
                label: metric.label,
                ...
                borderWidth: 2.5,
                pointRadius: trendRows.length === 1 ? 4 : 0,
                pointHoverRadius: 4,
                pointHitRadius: 14,
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    ...
                    callbacks: {
                        title(items) {
                            if (!isHourly) return items[0]?.label || '';
                            const hour = trendRows[items[0]?.dataIndex]?.hour;
                            return formatOverviewHourRange(hour);
                        },
                        label(context) {
...
function normalizedLeadViewRow(lead) {
    ...
    const adPlatform = lead.adPlatform || attribution.ad_platform || 'other';
    return {
        ...lead,
        status,
        adPlatform,
        ...
        metaCampaignId: attribution.meta_campaign_id || '',
        adSetName: attribution.utm_term || '',
        adSetId: attribution.meta_adset_id || '',
        adName: attribution.utm_content || '',
        adId: attribution.meta_ad_id || '',
        placement: attribution.placement || '',
        paidSource: attribution.utm_source || '',
...
function leadAttributionForPlatform(root = dashboardData?.leadAttribution, platform = activeLeadPlatform) {
    if (!root || typeof root !== 'object') return {};
    const platformSummary = root.platforms?.[platform];
    if (platformSummary && typeof platformSummary === 'object' && (platformSummary.recentLeads || platformSummary.byCampaign || platform === 'meta')) {
        return platformSummary;
    }
    return platform === 'google' ? root : {};
}

function leadReviewAttributionForPlatform(root = dashboardData?.leadAttribution, platform = activeLeadPlatform) {
    const attribution = leadAttributionForPlatform(root, platform);
    return attribution?.review && typeof attribution.review === 'object'
        ? attribution.review
        : attribution;
}

function setupLeadPlatformControls() {
    const buttons = document.querySelectorAll('[data-lead-platform]');
    if (!buttons.length) return;
    const saved = localStorage.getItem(LEAD_PLATFORM_STORAGE_KEY);
    activeLeadPlatform = saved === 'meta' ? 'meta' : 'google';

    buttons.forEach(button => {
        button.addEventListener('click', () => {
            const platform = button.dataset.leadPlatform === 'meta' ? 'meta' : 'google';
            if (platform === activeLeadPlatform) return;
            activeLeadPlatform = platform;
            localStorage.setItem(LEAD_PLATFORM_STORAGE_KEY, platform);
            renderAttribution();
        });
    });
}
...
function renderLeadReviewKanbanCard(row) {
    ...
    const detailLabel = row.adPlatform === 'meta' ? 'Ad set' : 'Keyword';
    const detail = row.adPlatform === 'meta'
        ? row.adSetName || row.adSetId || 'Ad set unavailable'
        : row.keyword || row.searchTerm || 'Keyword unavailable';
        ...
        <article class="lead-kanban-card" data-lead-session-key="${esc(row.sessionKey || '')}"
            ...
            <div class="lead-kanban-card-body">
                ...
                <div class="lead-kanban-source-block">
                    ...
                    <span>${esc(detailLabel)}${row.matchType ? ` · ${esc(row.matchType)}` : ''}</span>
                    <strong title="${esc(detail)}">${esc(detail)}</strong>
                </div>
...
function renderLeadCampaignCell(lead) {
    if (lead.adPlatform === 'meta') {
        const rows = [
            lead.campaignName || lead.utmCampaign
                ? `Campaign: ${lead.campaignName || lead.utmCampaign}`
                : 'Campaign unavailable',
            lead.metaCampaignId || lead.campaignId
                ? `Campaign ID: ${lead.metaCampaignId || lead.campaignId}`
...
function renderMetaAdSetCell(lead) {
    return `
        <div class="lead-source-cell">
            <strong>${esc(lead.adSetName ? `Ad set: ${lead.adSetName}` : 'Ad set unavailable')}</strong>
            <span>${esc(lead.adSetId ? `Ad-set ID: ${lead.adSetId}` : 'Ad-set ID unavailable')}</span>
        </div>
    `;
}

function renderMetaAdCell(lead) {
    return `
        <div class="lead-source-cell">
            <strong>${esc(lead.adName ? `Ad: ${lead.adName}` : 'Ad unavailable')}</strong>
            <span>${esc(lead.adId ? `Ad ID: ${lead.adId}` : 'Ad ID unavailable')}</span>
        </div>
    `;
}
...
function renderAttribution() {
    ...
    const leadAttributionRoot = dashboardData.leadAttribution || {};
    const leadAttribution = leadAttributionForPlatform(leadAttributionRoot);
    const leadReviewAttribution = leadReviewAttributionForPlatform(leadAttributionRoot);
    const isMeta = activeLeadPlatform === 'meta';
    const leadTotals = leadAttribution.totals || { uniqueLeads: 0, eventCount: 0, new: 0, useless: 0, maybe: 0, qualified: 0, qualifiedLost: 0, converted: 0, inProgress: 0, terminal: 0, qualifiedPipeline: 0 };
    const reviewTotals = leadReviewAttribution.totals || leadTotals;
    const journey = leadAttribution.journeySummary || { totalSessions: 0, sessionsWithMultipleActions: 0, topActionOverlaps: [], topPaths: [], recentJourneys: [] };
    const reviewRows = leadViewRows(leadReviewAttribution);
    leadProgressDetailsBySession.clear();
    reviewRows.forEach(row => {
        if (row.sessionKey) leadProgressDetailsBySession.set(String(row.sessionKey), row);
    });
    const needsReview = Number(reviewTotals.new ?? Math.max(Number(reviewTotals.uniqueLeads || 0) - Number(reviewTotals.terminal || 0) - Number(reviewTotals.inProgress || 0), 0));
    const scopedNeedsReview = Number(leadTotals.new ?? Math.max(Number(leadTotals.uniqueLeads || 0) - Number(leadTotals.terminal || 0) - Number(leadTotals.inProgress || 0), 0));
    const offlineExport = leadReviewAttribution.offlineExport || {};
    const safeguardTotals = leadReviewAttribution.safeguards || {};
    const exportLeads = currentLeadReviewExportLeads();

    document.querySelectorAll('.lead-platform-btn').forEach(button => {
        const active = button.dataset.leadPlatform === activeLeadPlatform;
        button.classList.toggle('active', active);
        button.setAttribute('aria-pressed', String(active));
    });
    const activityTitle = document.getElementById('leadReviewActivityTitle');
    const campaignTitle = document.getElementById('leadQualityCampaignTitle');
    const detailTitle = document.getElementById('leadQualityDetailTitle');
    if (activityTitle) activityTitle.textContent = `${isMeta ? 'Meta' : 'Google'} Lead Review & Activity · All time`;
    if (campaignTitle) campaignTitle.textContent = `${isMeta ? 'Meta' : 'Google'} Lead Quality by Campaign`;
    if (detailTitle) detailTitle.textContent = isMeta
        ? 'Lead Quality by Ad Set'
        : 'Lead Quality by Search Term / Keyword';

    if (els.attributionBadge) {
        els.attributionBadge.textContent = `${attribution.length} rows`;
    }
    if (els.leadAttributionBadge) {
        els.leadAttributionBadge.textContent = `${fmtNum(reviewTotals.uniqueLeads)} leads`;
    ...
    if (els.leadAttributionSummary) {
        const qualifiedRate = reviewTotals.uniqueLeads > 0 ? (reviewTotals.qualifiedPipeline / reviewTotals.uniqueLeads) * 100 : 0;
        const convertedRate = reviewTotals.uniqueLeads > 0 ? (reviewTotals.converted / reviewTotals.uniqueLeads) * 100 : 0;
        els.leadAttributionSummary.innerHTML = `
            <div class="insight-item" style="border-color: var(--primary)">
                <h4>Leads Waiting for Review</h4>
                <p>${fmtNum(needsReview)} of ${fmtNum(reviewTotals.uniqueLeads)} all-time leads require review.</p>
            </div>
            <div class="insight-item" style="border-color: var(--success)">
                <h4>Qualified Sales Leads</h4>
                <p>${fmtNum(reviewTotals.qualifiedPipeline)} leads are qualified (${fmtPct(qualifiedRate)}): ${fmtNum(reviewTotals.inProgress)} still open, ${fmtNum(reviewTotals.qualifiedLost)} lost, ${fmtNum(reviewTotals.converted)} won (${fmtPct(convertedRate)}).</p>
            </div>
            <div class="insight-item" style="border-color: var(--warning)">
                <h4>${isMeta ? 'Meta Attribution' : 'Offline Upload Readiness'}</h4>
                <p>${isMeta
        ? `${fmtNum(reviewTotals.uniqueLeads)} Meta leads retain campaign, ad-set, ad, and placement details when supplied by the landing URL.`
        : esc(exportReadinessMessage(offlineExport))}</p>
            </div>
            <div class="insight-item" style="border-color: var(--danger)">
                <h4>Qualification Safeguards</h4>
    ...
    if (els.leadJourneySummary) {
        ...
        els.leadJourneySummary.innerHTML = `
            <div class="insight-item" style="border-color: var(--info)">
                ...
                <p>${scopedNeedsReview > 0 ? `You have ${fmtNum(scopedNeedsReview)} new leads to check in this date range. Quality charts will update once you label them.` : 'All leads in this date range have been checked. Your lead quality charts are up to date!'}</p>
            </div>
            <div class="insight-item" style="border-color: var(--primary)">
    ...
    const commonLeadReviewColumns = [
        ...
        ...(isMeta ? [
            { field: 'adSetName', headerName: 'Ad Set', minWidth: 260, cellRenderer: p => renderMetaAdSetCell(p.data) },
            { field: 'adName', headerName: 'Ad', minWidth: 260, cellRenderer: p => renderMetaAdCell(p.data) },
            { field: 'placement', headerName: 'Placement', minWidth: 180, cellRenderer: p => leadFieldCell(p.value, 'Placement unavailable') },
            { field: 'paidSource', headerName: 'Meta Source', minWidth: 170, cellRenderer: p => leadFieldCell(p.value, 'Source unavailable') }
        ] : [
    ...
    initGrid('grid-leadReview', reviewRows, commonLeadReviewColumns);
    ...
    initGrid('grid-leadCampaigns', leadAttribution.byCampaign || [], [
        ...
        { field: 'campaignId', headerName: isMeta ? 'Campaign ID' : 'UTM Campaign' },
        ...(!isMeta ? [{ field: 'spend', headerName: 'Spend', valueFormatter: currencyFormatter, filter: 'agNumberColumnFilter' }] : []),
    ...
    initGrid('grid-leadSearchTerms', isMeta ? leadAttribution.byAdSet || [] : leadAttribution.bySearchTerm || [], [
        ...(isMeta ? [
            { field: 'adSetName', headerName: 'Ad Set', pinned: 'left', valueFormatter: p => p.value || p.data?.adSetId || '(none)' },
            { field: 'adSetId', headerName: 'Ad-set ID' },
            { field: 'campaignName', headerName: 'Campaign' }
        ] : [
...
async function handleLeadDeepLink() {
    ...
    try {
        ...
        const leadPlatform = data.lead?.adPlatform === 'meta' ? 'meta' : 'google';
        activeLeadPlatform = leadPlatform;
        localStorage.setItem(LEAD_PLATFORM_STORAGE_KEY, leadPlatform);
        dashboardData.leadAttribution.platforms = dashboardData.leadAttribution.platforms || {};
        const targetAttribution = dashboardData.leadAttribution.platforms[leadPlatform]
            || (dashboardData.leadAttribution.platforms[leadPlatform] = {
                platform: leadPlatform,
                recentLeads: leadPlatform === 'google'
                    ? dashboardData.leadAttribution.recentLeads || []
                    : []
            });
        const targetReview = targetAttribution.review && typeof targetAttribution.review === 'object'
            ? targetAttribution.review
            : targetAttribution;
        const rows = Array.isArray(targetReview.recentLeads) ? targetReview.recentLeads : [];
        if (!rows.some(row => row.sessionKey === data.lead.sessionKey)) {
            targetReview.recentLeads = [data.lead, ...rows].slice(0, 100);
        }
...
function applyPendingLeadStatus(sessionKey, status) {
    const attribution = dashboardData?.leadAttribution;
    const rowSets = [
        attribution?.recentLeads,
        attribution?.platforms?.google?.recentLeads,
        attribution?.platforms?.meta?.recentLeads,
        attribution?.platforms?.google?.review?.recentLeads,
        attribution?.platforms?.meta?.review?.recentLeads
    ];
