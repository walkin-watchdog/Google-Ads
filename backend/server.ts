...
async function initDB() {
    try {
        ...
        const semanticMemoryFailure = migrationResult.optionalFailures.find(failure => failure.id === '0002_initial_semantic_memory');
...
app.get('/api/account-controls/state', authenticateDashboard, async (req: Request, res: Response) => {
    try {
...
app.get('/api/account-controls/mutations/recent', authenticateDashboard, async (req: Request, res: Response) => {
    try {
...
app.post('/api/account-controls/mutations/preview', ...authenticateDashboardMutation, async (req: Request, res: Response) => {
    try {
...
app.post('/api/account-controls/mutations/keyword-preflight', ...authenticateDashboardMutation, async (req: Request, res: Response) => {
    try {
...
app.post('/api/account-controls/mutations/:id/confirm', ...authenticateDashboardMutation, async (req: Request, res: Response) => {
    try {
...
// API: Export the all-time first-party lead review queue.
    ...
    app.get('/', (_req: Request, res: Response) => {
        ...
        if (fs.existsSync(indexPath)) {
            ...
            res.setHeader(
                ...
                "default-src 'self'; connect-src 'self'; img-src 'self' data: blob:; style-src 'self' 'unsafe-inline'; font-src 'self' data:; script-src 'self' 'unsafe-inline'; worker-src 'self'; manifest-src 'self'; object-src 'none'; form-action 'self'; base-uri 'self'; frame-ancestors 'none'"
            );
