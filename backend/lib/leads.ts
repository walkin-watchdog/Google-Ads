...
export type LeadAdPlatform = 'google' | 'meta' | 'other';
...
function cleanAdTemplateValue(value: any): string | null {
    const text = clean(value);
    if (!text || /^\{\{[^{}]+\}\}$/.test(text)) return null;
    return text;
}

function normalizeLeadAdPlatform(value: any): LeadAdPlatform | null {
...
function detectLeadAdPlatform(input: {
    explicit?: any;
    utmSource?: any;
    utmMedium?: any;
    gclid?: any;
    ...
    const source = String(cleanAdTemplateValue(input.utmSource) || '').toLowerCase();
    const medium = String(cleanAdTemplateValue(input.utmMedium) || '').toLowerCase();
    if (
        cleanAdTemplateValue(input.gclid)
        || cleanAdTemplateValue(input.gbraid)
        || cleanAdTemplateValue(input.wbraid)
    ...
    if (
        medium === 'paid_social'
        && (
            ['meta', 'facebook', 'fb', 'instagram', 'ig', 'messenger', 'msg', 'audience_network', 'an'].includes(source)
            || cleanAdTemplateValue(input.metaCampaignId)
            || cleanAdTemplateValue(input.metaAdsetId)
            || cleanAdTemplateValue(input.metaAdId)
        )
    ) {
        return 'meta';
    }
    const explicit = normalizeLeadAdPlatform(input.explicit);
...
function isTrackingOnlyWebhook(body: any): boolean {
    return booleanFlag(body?.tracking_only ?? body?.trackingOnly) === true;
}
...
export function normalizeLeadWebhookPayload(raw: any): NormalizedLeadEvent {
    ...
    const metaAds = body.meta_ads || body.metaAds || {};
    const attribution = body.attribution || body.ads || {};
    const utmTerm = cleanAdTemplateValue(body.utm_term);
    const utmContent = cleanAdTemplateValue(body.utm_content);
    const metaCampaignId = firstClean(
        cleanAdTemplateValue(body.campaign_id),
        cleanAdTemplateValue(body.meta_campaign_id),
        cleanAdTemplateValue(body.metaCampaignId),
        cleanAdTemplateValue(metaAds.campaign_id),
        cleanAdTemplateValue(metaAds.campaignId),
        cleanAdTemplateValue(attribution.meta_campaign_id),
        cleanAdTemplateValue(attribution.metaCampaignId)
    );
    const metaAdsetId = firstClean(
        cleanAdTemplateValue(body.adset_id),
        cleanAdTemplateValue(body.meta_adset_id),
        cleanAdTemplateValue(body.metaAdsetId),
        cleanAdTemplateValue(metaAds.adset_id),
        cleanAdTemplateValue(metaAds.adsetId),
        cleanAdTemplateValue(attribution.meta_adset_id),
        cleanAdTemplateValue(attribution.metaAdsetId)
    );
    const metaAdId = firstClean(
        cleanAdTemplateValue(body.ad_id),
        cleanAdTemplateValue(body.meta_ad_id),
        cleanAdTemplateValue(body.metaAdId),
        cleanAdTemplateValue(metaAds.ad_id),
        cleanAdTemplateValue(metaAds.adId),
        cleanAdTemplateValue(attribution.meta_ad_id),
        cleanAdTemplateValue(attribution.metaAdId)
    );
    const adPlatform = detectLeadAdPlatform({
        explicit: firstClean(body.ad_platform, body.adPlatform, attribution.ad_platform, attribution.adPlatform),
    ...
    return {
        ...
        utm_source: cleanAdTemplateValue(body.utm_source),
        utm_medium: cleanAdTemplateValue(body.utm_medium),
        utm_campaign: cleanAdTemplateValue(body.utm_campaign),
        ...
        meta_campaign_id: adPlatform === 'meta' ? metaCampaignId : null,
        meta_adset_id: adPlatform === 'meta' ? metaAdsetId : null,
        meta_ad_id: adPlatform === 'meta' ? metaAdId : null,
        placement: adPlatform === 'meta'
            ? firstClean(
                cleanAdTemplateValue(body.placement),
                cleanAdTemplateValue(metaAds.placement),
                cleanAdTemplateValue(attribution.placement)
            )
            : null,
        utm_term: utmTerm,
        utm_content: utmContent,
        keyword: adPlatform === 'google'
            ? firstClean(
                body.keyword,
                body.keyword_text,
                body.keywordText,
            ...
            : null,
        match_type: adPlatform === 'google'
            ? normalizeMatchType(firstClean(
                body.match_type,
                body.matchType,
...
async function ensureLeadSchemaInternal(pool: Pool): Promise<void> {
    await pool.query(`
        CREATE TABLE IF NOT EXISTS lead_events (
            ...
            gbraid TEXT,
            wbraid TEXT,
            ad_platform VARCHAR(20) NOT NULL DEFAULT 'other',
            utm_source TEXT,
            utm_medium TEXT,
            utm_campaign TEXT,
            ad_group_id TEXT,
        ...
        CREATE INDEX IF NOT EXISTS lead_sessions_ad_platform_expr_idx ON lead_sessions ((attribution->>'ad_platform'));
        CREATE INDEX IF NOT EXISTS lead_sessions_meta_campaign_expr_idx ON lead_sessions ((attribution->>'meta_campaign_id'));
        CREATE INDEX IF NOT EXISTS lead_sessions_meta_adset_expr_idx ON lead_sessions ((attribution->>'meta_adset_id'));
        CREATE INDEX IF NOT EXISTS lead_sessions_meta_ad_expr_idx ON lead_sessions ((attribution->>'meta_ad_id'));

        CREATE INDEX IF NOT EXISTS lead_events_ad_platform_idx ON lead_events(ad_platform);
        CREATE INDEX IF NOT EXISTS lead_events_meta_campaign_id_idx ON lead_events(meta_campaign_id);
        CREATE INDEX IF NOT EXISTS lead_events_meta_adset_id_idx ON lead_events(meta_adset_id);
        CREATE INDEX IF NOT EXISTS lead_events_meta_ad_id_idx ON lead_events(meta_ad_id);
...
export async function rebuildLeadSession(pool: Queryable, sessionKey: string): Promise<void> {
    ...
    const attributionSource = reversedRows.find(row =>
        normalizeLeadAdPlatform(row.ad_platform) === 'google'
        || normalizeLeadAdPlatform(row.ad_platform) === 'meta'
    ) || reversedRows.find(row =>
        row.utm_campaign
        || row.ad_group_id
    ...
    const attributionPlatform = detectLeadAdPlatform({
        explicit: attributionSource.ad_platform,
        utmSource: attributionSource.utm_source,
        utmMedium: attributionSource.utm_medium,
        gclid: attributionSource.gclid,
        gbraid: attributionSource.gbraid,
        wbraid: attributionSource.wbraid,
        metaCampaignId: attributionSource.meta_campaign_id,
        metaAdsetId: attributionSource.meta_adset_id,
        metaAdId: attributionSource.meta_ad_id
    });
    const utmContent = clean(attributionSource.utm_content);
    ...
    await pool.query(
        ...
        [
            ...
            {
                ad_platform: attributionPlatform,
                utm_source: clean(attributionSource.utm_source),
                utm_medium: clean(attributionSource.utm_medium),
                utm_campaign: clean(attributionSource.utm_campaign),
                ad_group_id: attributionPlatform === 'google' ? clean(attributionSource.ad_group_id) : null,
                meta_campaign_id: attributionPlatform === 'meta' ? clean(attributionSource.meta_campaign_id) : null,
                meta_adset_id: attributionPlatform === 'meta' ? clean(attributionSource.meta_adset_id) : null,
                meta_ad_id: attributionPlatform === 'meta' ? clean(attributionSource.meta_ad_id) : null,
                placement: attributionPlatform === 'meta' ? clean(attributionSource.placement) : null,
                utm_term: clean(attributionSource.utm_term),
                utm_content: utmContent,
                keyword: attributionPlatform === 'google'
                    ? clean(attributionSource.keyword) || clean(attributionSource.utm_term)
                    : null,
                match_type: attributionPlatform === 'google'
                    ? clean(attributionSource.match_type) || matchTypeFromUtmContent(utmContent)
                    : null,
                gclid: attributionPlatform === 'google' ? clean(attributionSource.gclid) : null,
                gbraid: attributionPlatform === 'google' ? clean(attributionSource.gbraid) : null,
                wbraid: attributionPlatform === 'google' ? clean(attributionSource.wbraid) : null
            }
        ]
    );
}
    ...
    await client.query(
        `INSERT INTO lead_events
         (event_id, session_key, session_key_type, kind, lead_id, session_id, gclid, gbraid, wbraid,
          ad_platform, utm_source, utm_medium, utm_campaign, ad_group_id, meta_campaign_id, meta_adset_id, meta_ad_id, placement,
          utm_term, utm_content, keyword, match_type, name, email, phone,
         ...
         ON CONFLICT (event_id) DO UPDATE SET
            ...
            gclid = CASE
                WHEN EXCLUDED.ad_platform = 'meta' THEN NULL
                ELSE COALESCE(EXCLUDED.gclid, lead_events.gclid)
            END,
            gbraid = CASE
                WHEN EXCLUDED.ad_platform = 'meta' THEN NULL
                ELSE COALESCE(EXCLUDED.gbraid, lead_events.gbraid)
            END,
            wbraid = CASE
                WHEN EXCLUDED.ad_platform = 'meta' THEN NULL
                ELSE COALESCE(EXCLUDED.wbraid, lead_events.wbraid)
            END,
            ad_platform = CASE
                WHEN EXCLUDED.ad_platform <> 'other' THEN EXCLUDED.ad_platform
                ELSE lead_events.ad_platform
            END,
            utm_source = CASE
                WHEN EXCLUDED.ad_platform = 'other'
                 AND lead_events.ad_platform <> 'other'
                THEN lead_events.utm_source
                WHEN EXCLUDED.ad_platform <> 'other'
                 AND EXCLUDED.ad_platform <> lead_events.ad_platform
                THEN EXCLUDED.utm_source
                ELSE COALESCE(EXCLUDED.utm_source, lead_events.utm_source)
            END,
            utm_medium = CASE
                WHEN EXCLUDED.ad_platform = 'other'
                 AND lead_events.ad_platform <> 'other'
                THEN lead_events.utm_medium
                WHEN EXCLUDED.ad_platform <> 'other'
                 AND EXCLUDED.ad_platform <> lead_events.ad_platform
                THEN EXCLUDED.utm_medium
                ELSE COALESCE(EXCLUDED.utm_medium, lead_events.utm_medium)
            END,
            utm_campaign = CASE
                WHEN EXCLUDED.ad_platform = 'other'
                 AND lead_events.ad_platform <> 'other'
                THEN lead_events.utm_campaign
                WHEN EXCLUDED.ad_platform <> 'other'
                 AND EXCLUDED.ad_platform <> lead_events.ad_platform
                THEN EXCLUDED.utm_campaign
                ELSE COALESCE(EXCLUDED.utm_campaign, lead_events.utm_campaign)
            END,
            ad_group_id = CASE
                WHEN EXCLUDED.ad_platform = 'meta' THEN NULL
                WHEN EXCLUDED.ad_platform = 'other'
                 AND lead_events.ad_platform <> 'other'
                THEN lead_events.ad_group_id
                ELSE COALESCE(EXCLUDED.ad_group_id, lead_events.ad_group_id)
            END,
            meta_campaign_id = CASE
                WHEN EXCLUDED.ad_platform = 'google' THEN NULL
                WHEN EXCLUDED.ad_platform = 'other'
                 AND lead_events.ad_platform <> 'other'
                THEN lead_events.meta_campaign_id
                ELSE COALESCE(EXCLUDED.meta_campaign_id, lead_events.meta_campaign_id)
            END,
            meta_adset_id = CASE
                WHEN EXCLUDED.ad_platform = 'google' THEN NULL
                WHEN EXCLUDED.ad_platform = 'other'
                 AND lead_events.ad_platform <> 'other'
                THEN lead_events.meta_adset_id
                ELSE COALESCE(EXCLUDED.meta_adset_id, lead_events.meta_adset_id)
            END,
            meta_ad_id = CASE
                WHEN EXCLUDED.ad_platform = 'google' THEN NULL
                WHEN EXCLUDED.ad_platform = 'other'
                 AND lead_events.ad_platform <> 'other'
                THEN lead_events.meta_ad_id
                ELSE COALESCE(EXCLUDED.meta_ad_id, lead_events.meta_ad_id)
            END,
            placement = CASE
                WHEN EXCLUDED.ad_platform = 'google' THEN NULL
                WHEN EXCLUDED.ad_platform = 'other'
                 AND lead_events.ad_platform <> 'other'
                THEN lead_events.placement
                ELSE COALESCE(EXCLUDED.placement, lead_events.placement)
            END,
            utm_term = CASE
                WHEN EXCLUDED.ad_platform = 'other'
                 AND lead_events.ad_platform <> 'other'
                THEN lead_events.utm_term
                WHEN EXCLUDED.ad_platform <> 'other'
                 AND EXCLUDED.ad_platform <> lead_events.ad_platform
                THEN EXCLUDED.utm_term
                ELSE COALESCE(EXCLUDED.utm_term, lead_events.utm_term)
            END,
            utm_content = CASE
                WHEN EXCLUDED.ad_platform = 'other'
                 AND lead_events.ad_platform <> 'other'
                THEN lead_events.utm_content
                WHEN EXCLUDED.ad_platform <> 'other'
                 AND EXCLUDED.ad_platform <> lead_events.ad_platform
                THEN EXCLUDED.utm_content
                ELSE COALESCE(EXCLUDED.utm_content, lead_events.utm_content)
            END,
            keyword = CASE
                WHEN EXCLUDED.ad_platform = 'meta' THEN NULL
                WHEN EXCLUDED.ad_platform = 'other'
                 AND lead_events.ad_platform <> 'other'
                THEN lead_events.keyword
                ELSE COALESCE(EXCLUDED.keyword, lead_events.keyword)
            END,
            match_type = CASE
                WHEN EXCLUDED.ad_platform = 'meta' THEN NULL
                WHEN EXCLUDED.ad_platform = 'other'
                 AND lead_events.ad_platform <> 'other'
                THEN lead_events.match_type
                ELSE COALESCE(EXCLUDED.match_type, lead_events.match_type)
            END,
...
function leadPlatformSql(attributionExpression = 'attribution'): string {
    return `COALESCE(
        NULLIF(${attributionExpression}->>'ad_platform', ''),
        CASE
            WHEN NULLIF(${attributionExpression}->>'gclid', '') IS NOT NULL
              OR NULLIF(${attributionExpression}->>'gbraid', '') IS NOT NULL
              OR NULLIF(${attributionExpression}->>'wbraid', '') IS NOT NULL
              OR (
                LOWER(COALESCE(${attributionExpression}->>'utm_source', '')) = 'google'
                AND LOWER(COALESCE(${attributionExpression}->>'utm_medium', '')) = 'cpc'
              )
            THEN 'google'
            WHEN LOWER(COALESCE(${attributionExpression}->>'utm_medium', '')) = 'paid_social'
              AND (
                LOWER(COALESCE(${attributionExpression}->>'utm_source', '')) IN (
                  'meta', 'facebook', 'fb', 'instagram', 'ig',
                  'messenger', 'msg', 'audience_network', 'an'
                )
                OR NULLIF(${attributionExpression}->>'meta_campaign_id', '') IS NOT NULL
                OR NULLIF(${attributionExpression}->>'meta_adset_id', '') IS NOT NULL
                OR NULLIF(${attributionExpression}->>'meta_ad_id', '') IS NOT NULL
              )
            THEN 'meta'
            ELSE 'other'
        END
...
function sessionDateWhere(
    ...
    platform: LeadAdPlatform | null = null
    ...
    if (platform) {
        ...
        conditions.push(`${leadPlatformSql()} = $${params.length}`);
    }
    return {
...
function buildLeadRows(
    ...
    return sessions.map(session => {
        ...
        const metaCampaignId = clean(attribution.meta_campaign_id) || firstEventValue(sessionEvents, 'meta_campaign_id');
        const metaAdsetId = clean(attribution.meta_adset_id) || firstEventValue(sessionEvents, 'meta_adset_id');
        const metaAdId = clean(attribution.meta_ad_id) || firstEventValue(sessionEvents, 'meta_ad_id');
        const adPlatform = detectLeadAdPlatform({
            explicit: clean(attribution.ad_platform) || firstEventValue(sessionEvents, 'ad_platform'),
            utmSource: clean(attribution.utm_source) || firstEventValue(sessionEvents, 'utm_source'),
            utmMedium: clean(attribution.utm_medium) || firstEventValue(sessionEvents, 'utm_medium'),
        ...
        const campaign = adPlatform === 'google' && rawCampaignId ? spendByCampaign.get(rawCampaignId) : null;
        const campaignId = adPlatform === 'meta'
            ? metaCampaignId || rawCampaignId
            : campaign?.campaignId || rawCampaignId;
        const mergedAttribution = {
            ...
            meta_ad_id: adPlatform === 'meta' ? metaAdId : null,
            placement: adPlatform === 'meta'
                ? clean(attribution.placement) || firstEventValue(sessionEvents, 'placement')
                : null,
            utm_term: utmTerm,
            utm_content: utmContent,
            keyword: adPlatform === 'google'
                ? clean(attribution.keyword) || firstEventValue(sessionEvents, 'keyword') || utmTerm
                : null,
            match_type: adPlatform === 'google'
                ? clean(attribution.match_type) || firstEventValue(sessionEvents, 'match_type') || matchTypeFromUtmContent(utmContent)
                : null,
            gclid: adPlatform === 'google'
                ? clean(attribution.gclid) || firstEventValue(sessionEvents, 'gclid')
                : null,
            gbraid: adPlatform === 'google'
                ? clean(attribution.gbraid) || firstEventValue(sessionEvents, 'gbraid')
                : null,
            wbraid: adPlatform === 'google'
                ? clean(attribution.wbraid) || firstEventValue(sessionEvents, 'wbraid')
                : null
        };
        ...
        return {
            ...
            offlineConversionReady: adPlatform === 'google'
                && ['qualified', 'converted'].includes(normalizeStatus(session.status))
                && hasClickId(mergedAttribution)
                && !safeguardBlocked,
...
async function aggregateLeadBucketForRange(
    pool: Pool,
    range: { start: string; end: string } | null,
    scope: LeadAttributionScope,
    platform: LeadAdPlatform
): Promise<any> {
    if (!range) return emptyLeadBucket();
    const { where, params } = sessionDateWhere({ start: range.start, end: range.end }, scope, platform);
    const { rows } = await pool.query(
...
async function buildLeadAttributionOverviewSummary(pool: Pool, dashboardData: any): Promise<any> {
    ...
    const scope = dashboardLeadScope(dashboardData);
    const { where, params } = sessionDateWhere(dateRange, scope, 'google');
    const metaScope: LeadAttributionScope = {
        campaignId: null,
        campaignNames: [],
        adGroupId: null,
        adGroupNames: []
    };
    const { where: metaWhere, params: metaParams } = sessionDateWhere(dateRange, metaScope, 'meta');
    ...
    const [
        ...
        aggregateLeadBucketForRange(pool, periodRange(dashboardData?.periodComparison?.previousPeriod?.label), scope, 'google'),
        aggregateLeadBucketForRange(pool, periodRange(dashboardData?.periodComparison?.currentPeriod?.label), scope, 'google'),
        pool.query(
            `SELECT status, COUNT(*)::int AS unique_leads, COALESCE(SUM(event_count), 0)::int AS event_count
             FROM lead_sessions
             ${metaWhere}
    ...
    const metaTotals = bucketFromStatusRows(metaTotalsResult.rows);
    ...
    const summary = {
        ...
        ...googleOverviewSummary,
        platforms: {
            google: googleOverviewSummary,
            meta: {
                platform: 'meta',
                totals: metaTotals,
                scope: {
...
export function buildPlatformLeadAttributionSummary(input: {
    platform: LeadAdPlatform;
    sessions: any[];
    events: any[];
    spendByCampaign: Map<string, { campaignId: string; campaignName: string | null; spend: number }>;
    ...
    const { platform, sessions, events, spendByCampaign, scope } = input;
    const totals = emptyLeadBucket();
    const byCampaign = new Map<string, any>();
    const bySearchTerm = new Map<string, any>();
    const byAdSet = new Map<string, any>();
    const byAd = new Map<string, any>();
    const journeySummary = buildLeadJourneySummary(sessions, events);
    const leadRows = buildLeadRows(sessions, events, journeySummary, spendByCampaign);

    for (const lead of leadRows) {
        bumpLeadBucket(totals, lead);
        const attribution = lead.attribution || {};
        const rawCampaignName = clean(attribution.utm_campaign);
        const campaignId = platform === 'meta'
            ? clean(attribution.meta_campaign_id) || rawCampaignName || '(none)'
            : clean(lead?.campaign?.campaignId) || rawCampaignName || '(none)';
        const campaignSpend = platform === 'google'
            ? spendByCampaign.get(campaignId) || (rawCampaignName ? spendByCampaign.get(rawCampaignName) : undefined)
            : undefined;
        const campaignBucket = byCampaign.get(campaignId) || emptyLeadBucket({
            campaignId,
            campaignName: platform === 'meta' ? rawCampaignName : campaignSpend?.campaignName || null,
            spend: campaignSpend?.spend || 0,
            trueCpa: 0,
            qualifiedCpa: 0,
            convertedCpa: 0,
        ...
        bumpLeadBucket(campaignBucket, lead);
        byCampaign.set(campaignId, campaignBucket);

        if (platform === 'meta') {
            const adSetId = clean(attribution.meta_adset_id) || clean(attribution.utm_term) || '(none)';
            // Key includes campaignId so adsets/ads with identical names or missing IDs
            // in different campaigns are never merged into the same bucket.
            const adSetKey = `${campaignId}|${adSetId}`;
            const adSetBucket = byAdSet.get(adSetKey) || emptyLeadBucket({
                campaignId,
                campaignName: rawCampaignName,
                adSetId,
                adSetName: clean(attribution.utm_term)
            });
            bumpLeadBucket(adSetBucket, lead);
            byAdSet.set(adSetKey, adSetBucket);

            const adId = clean(attribution.meta_ad_id) || clean(attribution.utm_content) || '(none)';
            const placement = clean(attribution.placement);
            const adKey = `${campaignId}|${adSetId}|${adId}|${placement || ''}`;
            const adBucket = byAd.get(adKey) || emptyLeadBucket({
                campaignId,
                campaignName: rawCampaignName,
                adSetId,
                adSetName: clean(attribution.utm_term),
                adId,
                adName: clean(attribution.utm_content),
                placement
            });
            bumpLeadBucket(adBucket, lead);
            byAd.set(adKey, adBucket);
            continue;
        }
        ...
        bumpLeadBucket(termBucket, lead);
    ...
    return {
        ...
        byCampaign: campaigns,
        bySearchTerm: Array.from(bySearchTerm.values()).sort((a, b) => b.uniqueLeads - a.uniqueLeads).slice(0, 100),
        byAdSet: Array.from(byAdSet.values()).sort((a, b) => b.uniqueLeads - a.uniqueLeads).slice(0, 100),
        byAd: Array.from(byAd.values()).sort((a, b) => b.uniqueLeads - a.uniqueLeads).slice(0, 100),
        journeySummary,
        recentLeads: leadRows.slice(0, 50),
        safeguards: qualificationSafeguardCountsFromLeads(leadRows),
        offlineExport: platform === 'google'
            ? buildOfflineExportReadiness(leadRows)
            : {
                statuses: [],
                readyRows: 0,
                skippedMissingClickId: 0,
                qualifiedOrConverted: 0,
                needsReview: Number(totals.new || 0) + Number(totals.maybe || 0),
                supported: false
            }
    };
}
...
export async function getLeadAttributionSummary(
    ...
    const googleQuery = sessionDateWhere(dateRange, scope, 'google');
    const metaQuery = sessionDateWhere(dateRange, metaScope, 'meta');
    // Lead Review is an operational queue, so it must remain all-time even when
    // the analytical attribution summaries are scoped to a dashboard date range.
    // Campaign/ad-group and platform filters still apply to the queue.
    const googleReviewQuery = sessionDateWhere({ start: null, end: null }, scope, 'google');
    const metaReviewQuery = sessionDateWhere({ start: null, end: null }, metaScope, 'meta');
    const sessionSelect = `SELECT session_key, session_key_type, status, status_rank, progress_status, progress_trigger, progress_revision, progress_answered_count, qualification_progress, lead_source, event_count, lead_ids, attribution, contact, first_seen, last_seen, updated_at
             FROM lead_sessions`;
    const [googleResult, metaResult, googleReviewResult, metaReviewResult] = await Promise.all([
        pool.query(
            `${sessionSelect}
             ${googleQuery.where}
             ORDER BY last_seen DESC`,
        ...
        pool.query(
            `${sessionSelect}
             ${googleReviewQuery.where}
             ORDER BY last_seen DESC`,
            googleReviewQuery.params
        ),
        pool.query(
            `${sessionSelect}
             ${metaReviewQuery.where}
             ORDER BY last_seen DESC`,
            metaReviewQuery.params
        )
    ]);
    const combinedRows = Array.from(new Map(
        [...googleReviewResult.rows, ...metaReviewResult.rows, ...googleResult.rows, ...metaResult.rows]
            .map((row: any) => [row.session_key, row])
    ).values());
    const eventsResult = combinedRows.length
        ? await pool.query(
            `SELECT event_id, session_key, kind, source, lead_id, session_id, tracking_only, status, submitted_at, received_at,
                    gclid, gbraid, wbraid, ad_platform, utm_source, utm_medium, utm_campaign, ad_group_id,
                    meta_campaign_id, meta_adset_id, meta_ad_id, placement, utm_term, utm_content,
            ...
            [combinedRows.map((row: any) => row.session_key)]
        )
        : { rows: [] };
    const spendByCampaign = campaignSpendMap(dashboardData);
    const googleSessionKeys = new Set(googleResult.rows.map((session: any) => session.session_key));
    const metaSessionKeys = new Set(metaResult.rows.map((session: any) => session.session_key));
    const googleReviewSessionKeys = new Set(googleReviewResult.rows.map((session: any) => session.session_key));
    const metaReviewSessionKeys = new Set(metaReviewResult.rows.map((session: any) => session.session_key));
    const googleEvents = eventsResult.rows.filter((event: any) =>
        googleSessionKeys.has(event.session_key)
    );
    const metaEvents = eventsResult.rows.filter((event: any) =>
        metaSessionKeys.has(event.session_key)
    );
    const googleReviewEvents = eventsResult.rows.filter((event: any) =>
        googleReviewSessionKeys.has(event.session_key)
    );
    const metaReviewEvents = eventsResult.rows.filter((event: any) =>
        metaReviewSessionKeys.has(event.session_key)
    );
    const googleSummary = buildPlatformLeadAttributionSummary({
        platform: 'google',
        sessions: googleResult.rows,
        events: googleEvents,
        spendByCampaign,
        scope: leadAttributionScopeSummary(scope)
    });
    const metaSummary = buildPlatformLeadAttributionSummary({
        platform: 'meta',
        sessions: metaResult.rows,
        events: metaEvents,
    ...
    const googleReviewSummary = buildPlatformLeadAttributionSummary({
        platform: 'google',
        sessions: googleReviewResult.rows,
        events: googleReviewEvents,
        spendByCampaign,
        scope: leadAttributionScopeSummary(scope)
    });
    const metaReviewSummary = buildPlatformLeadAttributionSummary({
        platform: 'meta',
        sessions: metaReviewResult.rows,
        events: metaReviewEvents,
        spendByCampaign: new Map(),
        scope: {
            level: 'account',
            dateRangeOnly: false
    ...
    const reviewProjection = (summary: any) => ({
        platform: summary.platform,
        totals: summary.totals,
        recentLeads: summary.recentLeads,
        safeguards: summary.safeguards,
        offlineExport: summary.offlineExport,
        scope: summary.scope,
        allTime: true
    });
    const googleWithReview = {
        ...googleSummary,
        review: reviewProjection(googleReviewSummary)
    };
    const metaWithReview = {
        ...metaSummary,
        review: reviewProjection(metaReviewSummary)
    };
    return {
        ...googleWithReview,
        generatedAt: new Date().toISOString(),
        dateRange,
        allLeads: buildLeadRows(
            googleResult.rows,
            googleEvents,
            googleSummary.journeySummary,
            spendByCampaign
        ),
        recentSessions: googleResult.rows.slice(0, 50),
        platforms: {
            google: googleWithReview,
            meta: metaWithReview
        }
    };
}
...
export async function exportLeadReviewCsv(pool: Pool, options: {
    ...
    const params: any[] = [];
    const conditions: string[] = [];
    const platform = normalizeLeadAdPlatform(options.platform) === 'meta' ? 'meta' : 'google';
    params.push(platform);
    conditions.push(`${leadPlatformSql()} = $${params.length}`);
    if (startDate) {
    ...
    for (const lead of leadRows) {
        ...
        csvRows.push([
            ...
            spreadsheetSafeText(clean(lead.adPlatform) || ''),
            ...
            spreadsheetSafeText(clean(attribution.meta_campaign_id) || ''),
            spreadsheetSafeText(clean(attribution.meta_adset_id) || ''),
            spreadsheetSafeText(clean(attribution.meta_ad_id) || ''),
            spreadsheetSafeText(clean(attribution.placement) || ''),
...
export async function exportOfflineConversionsCsv(pool: Pool, options: OfflineConversionExportOptions = {}): Promise<{
    ...
    const conditions = [
        ...
        `${leadPlatformSql('ls.attribution')} = 'google'`
    ];
    const startDate = cleanDate(options.startDate);
    const endDate = cleanDate(options.endDate);
    if (startDate) {
