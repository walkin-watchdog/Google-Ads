...
function defaultLiveAttachOptions(view: DashboardPayloadView): AttachLiveDashboardOptions {
    return {
        ...
        // The attribution Journeys sub-tab needs lead_events to build action paths.
        // Overview mode intentionally reads only lead_sessions aggregates and returns
        // empty journey edges, so use the full lead summary for this view.
        leadMode: view === 'attribution'
            ? 'full'
            : ['overview', 'keywords', 'rank', 'proposals'].includes(view) ? 'overview' : 'none'
    };
}
