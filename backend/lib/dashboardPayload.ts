...
function dashboardViewPayloadMemoryKey(view: DashboardPayloadView, filters: DashboardFilters, watermark: string): string {
    return `${dashboardCacheKey(filters)}:view=${view}:watermark=${watermark}`;
}
...
function getMemoryCachedDashboardViewPayload(view: DashboardPayloadView, filters: DashboardFilters, watermark: string): any | null {
    const ttlMs = dashboardViewCacheTtlMs();
    if (ttlMs <= 0) return null;
    const key = dashboardViewPayloadMemoryKey(view, filters, watermark);
...
function setMemoryCachedDashboardViewPayload(view: DashboardPayloadView, filters: DashboardFilters, watermark: string, payload: any): void {
    const ttlMs = dashboardViewCacheTtlMs();
    if (ttlMs <= 0) return;
    ...
    const now = Date.now();
    dashboardViewPayloadMemoryCache.set(dashboardViewPayloadMemoryKey(view, filters, watermark), {
...
export type DashboardPayloadView = 'full' | 'overview' | 'performance' | 'keywords' | 'attribution' | 'rank' | 'proposals';
...
function normalizeLeadStatus(value: any): string {
    ...
    return ['new', 'qualified', 'converted', 'qualified_lost', 'useless'].includes(normalized) ? normalized : 'new';
}
...
export async function resolveDashboardFilters(pool: Pool, raw: Record<string, any> = {}): Promise<DashboardFilters> {
    ...
    const endDate = one(raw.endDate) || today();
    const startDate = one(raw.startDate) || addDays(endDate, -29);
...
function buildPayloadFromBundleSync(
    ...
    const dailyTrend = aggregateBy(bundle.adGroupDaily.length ? bundle.adGroupDaily : bundle.campaignDaily, row => row.date, row => ({ date: row.date }))
    ...
    const clickPaths = bundle.clickEvidenceDaily.map(row => ({
        date: row.date,
    ...
    const auctionInsights = bundle.auctionInsightsRows.map(row => ({
        date: row.auction_date || null,
        campaign: row.campaign_name || null,
        campaignId: row.campaign_id || null,
        adGroup: row.ad_group_name || null,
        adGroupId: row.ad_group_id || null,
        domain: row.domain,
        isYou: normKey(row.domain) === 'you',
        impressionShare: pctFraction(row.impression_share),
        overlapRate: pctFraction(row.overlap_rate),
        positionAboveRate: pctFraction(row.position_above_rate),
        outrankingShare: pctFraction(row.outranking_share),
        topImpressionRate: pctFraction(row.top_impression_percentage),
        absoluteTopImpressionRate: pctFraction(row.absolute_top_impression_percentage),
...
export async function buildDashboardPayloadForView(
    ...
    const inflightKey = `${dashboardCacheKey(filters)}:view=${view}:watermark=${inflightWatermark}:bypass=${options.bypassViewCache ? '1' : '0'}`;
...
async function buildDashboardPayloadForResolvedView(
    ...
    const cachedPayload = options.bypassViewCache
        ? null
        : await timed(options.timings, 'view_cache', async () => getMemoryCachedDashboardViewPayload(view, filters, watermark));
    if (cachedPayload) {
        cachedPayload.meta = {
            ...(cachedPayload.meta || {}),
    ...
    const liveOptions: AttachLiveDashboardOptions = {
    ...
    setMemoryCachedDashboardViewPayload(view, filters, watermark, projected);
    return projected;
}
