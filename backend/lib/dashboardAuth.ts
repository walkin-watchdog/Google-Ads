...
import { ensureDashboardUsersSchema, type DashboardUserRow } from './dashboardUsers';
...
export async function ensureDashboardAuthSchema(pool: Pool): Promise<void> {
  ...
  await pool.query(`
        CREATE TABLE IF NOT EXISTS dashboard_sessions (
            ...
            auth_method TEXT NOT NULL DEFAULT 'magic'
                CONSTRAINT dashboard_sessions_auth_method_chk CHECK (auth_method IN ('magic','user')),
            csrf_hash TEXT,
            expires_at TIMESTAMPTZ NOT NULL,
            ...
            ip_address TEXT,
            user_agent TEXT,
            CONSTRAINT dashboard_sessions_user_auth_chk CHECK (
  ...
  await pool.query(`CREATE INDEX IF NOT EXISTS dashboard_sessions_user_id_idx ON dashboard_sessions(user_id)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS dashboard_sessions_idle_expires_idx ON dashboard_sessions(idle_expires_at)`);
  await pool.query(`
