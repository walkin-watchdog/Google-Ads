...
export async function ensureSemanticMemorySchema(pool: Pool): Promise<void> {
    try {
        await pool.query(`
            ...
            ALTER TABLE semantic_memories ADD COLUMN IF NOT EXISTS memory_id UUID DEFAULT gen_random_uuid();
            UPDATE semantic_memories SET memory_id = gen_random_uuid() WHERE memory_id IS NULL;
            DO $$
            BEGIN
                IF NOT EXISTS (
                    SELECT 1
                    FROM pg_constraint
                    WHERE conrelid = 'semantic_memories'::regclass
                      AND contype = 'p'
                ) THEN
                    ALTER TABLE semantic_memories ADD PRIMARY KEY (memory_id);
                END IF;
            END;
            $$;

            ALTER TABLE semantic_memories ADD COLUMN IF NOT EXISTS customer_id VARCHAR(50);
            ALTER TABLE semantic_memories ADD COLUMN IF NOT EXISTS scope_type VARCHAR(50);
            ALTER TABLE semantic_memories ADD COLUMN IF NOT EXISTS customer_resource_name TEXT;
            ALTER TABLE semantic_memories ADD COLUMN IF NOT EXISTS campaign_resource_name TEXT;
            ALTER TABLE semantic_memories ADD COLUMN IF NOT EXISTS ad_group_resource_name TEXT;
            ALTER TABLE semantic_memories ADD COLUMN IF NOT EXISTS criterion_resource_name TEXT;
            ALTER TABLE semantic_memories ADD COLUMN IF NOT EXISTS proposal_id VARCHAR(100);
            ALTER TABLE semantic_memories ADD COLUMN IF NOT EXISTS related_proposal_id VARCHAR(100);
            ALTER TABLE semantic_memories ADD COLUMN IF NOT EXISTS search_term TEXT;
            ALTER TABLE semantic_memories ADD COLUMN IF NOT EXISTS search_term_normalized TEXT;
            ALTER TABLE semantic_memories ADD COLUMN IF NOT EXISTS scope_hash VARCHAR(64);
            ALTER TABLE semantic_memories ADD COLUMN IF NOT EXISTS campaign_id VARCHAR(100);
            ALTER TABLE semantic_memories ADD COLUMN IF NOT EXISTS ad_group_id VARCHAR(100);
            ALTER TABLE semantic_memories ADD COLUMN IF NOT EXISTS criterion_id VARCHAR(100);
            ALTER TABLE semantic_memories ADD COLUMN IF NOT EXISTS category VARCHAR(80);
            ALTER TABLE semantic_memories ADD COLUMN IF NOT EXISTS content TEXT;
            ALTER TABLE semantic_memories ADD COLUMN IF NOT EXISTS content_hash VARCHAR(64);
            ALTER TABLE semantic_memories ADD COLUMN IF NOT EXISTS verification_status VARCHAR(50);
            ALTER TABLE semantic_memories ADD COLUMN IF NOT EXISTS authority VARCHAR(50);
            ALTER TABLE semantic_memories ADD COLUMN IF NOT EXISTS source VARCHAR(80);
            ALTER TABLE semantic_memories ADD COLUMN IF NOT EXISTS source_ref TEXT;
            ALTER TABLE semantic_memories ADD COLUMN IF NOT EXISTS is_active BOOLEAN DEFAULT TRUE;
            ALTER TABLE semantic_memories ADD COLUMN IF NOT EXISTS version INTEGER DEFAULT 1;
            ALTER TABLE semantic_memories ADD COLUMN IF NOT EXISTS valid_until TIMESTAMP WITH TIME ZONE;
            ALTER TABLE semantic_memories ADD COLUMN IF NOT EXISTS supersedes_memory_id UUID REFERENCES semantic_memories(memory_id);
            ALTER TABLE semantic_memories ADD COLUMN IF NOT EXISTS exception_to_memory_id UUID REFERENCES semantic_memories(memory_id);
            ALTER TABLE semantic_memories ADD COLUMN IF NOT EXISTS deactivated_at TIMESTAMP WITH TIME ZONE;
            ALTER TABLE semantic_memories ADD COLUMN IF NOT EXISTS deactivation_reason TEXT;
            ALTER TABLE semantic_memories ADD COLUMN IF NOT EXISTS deactivated_by VARCHAR(120);
            ALTER TABLE semantic_memories ADD COLUMN IF NOT EXISTS created_by VARCHAR(120);
            ALTER TABLE semantic_memories ADD COLUMN IF NOT EXISTS created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP;
            ALTER TABLE semantic_memories ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP;
            ALTER TABLE semantic_memories ALTER COLUMN is_active SET DEFAULT TRUE;
            ALTER TABLE semantic_memories ALTER COLUMN version SET DEFAULT 1;
            UPDATE semantic_memories SET is_active = TRUE WHERE is_active IS NULL;
            UPDATE semantic_memories SET version = 1 WHERE version IS NULL;
            ...
            ALTER TABLE semantic_memory_embeddings_1536 ADD COLUMN IF NOT EXISTS memory_id UUID REFERENCES semantic_memories(memory_id) ON DELETE CASCADE;
            ALTER TABLE semantic_memory_embeddings_1536 ADD COLUMN IF NOT EXISTS customer_id VARCHAR(50);
            ALTER TABLE semantic_memory_embeddings_1536 ADD COLUMN IF NOT EXISTS embedding_model VARCHAR(120);
            ALTER TABLE semantic_memory_embeddings_1536 ADD COLUMN IF NOT EXISTS embedding VECTOR(1536);
            ALTER TABLE semantic_memory_embeddings_1536 ADD COLUMN IF NOT EXISTS created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP;
            DELETE FROM semantic_memory_embeddings_1536 WHERE memory_id IS NULL;
            DO $$
            BEGIN
                IF NOT EXISTS (
                    SELECT 1
                    FROM pg_constraint
                    WHERE conrelid = 'semantic_memory_embeddings_1536'::regclass
                      AND contype = 'p'
                ) THEN
                    ALTER TABLE semantic_memory_embeddings_1536 ADD PRIMARY KEY (memory_id);
                END IF;
            END;
            $$;
            ...
            DELETE FROM semantic_memory_embeddings_1536 e
            USING semantic_memories m
            WHERE e.memory_id = m.memory_id AND m.is_active = FALSE;
        `);
        await repairSemanticMemorySchemaData(pool);
    } catch (err: any) {
...
async function repairSemanticMemorySchemaData(pool: Pool): Promise<void> {
    await pool.query(`
        UPDATE semantic_memory_embeddings_1536 e
        SET customer_id = m.customer_id
        FROM semantic_memories m
        WHERE e.memory_id = m.memory_id
          AND m.customer_id IS NOT NULL
          AND e.customer_id IS DISTINCT FROM m.customer_id;
    ...
    const { rows } = await pool.query(`
        SELECT memory_id, customer_id, scope_type, customer_resource_name, campaign_resource_name,
               ad_group_resource_name, criterion_resource_name, proposal_id, related_proposal_id,
               search_term, search_term_normalized, content, content_hash, scope_hash
        FROM semantic_memories
        WHERE customer_id IS NOT NULL
          AND (
            customer_resource_name IS NULL
            OR scope_hash IS NULL
            OR content_hash IS NULL
            OR (search_term IS NOT NULL AND search_term_normalized IS NULL)
          )
    `);
    ...
    for (const row of rows) {
        ...
        const customerResourceName = row.customer_resource_name || `customers/${customerId}`;
        const searchTermNormalized = row.search_term
            ? normalizeSearchTerm(row.search_term)
            : row.search_term_normalized || null;
        const contentHash = row.content_hash || sha256(String(row.content || '').trim().replace(/\s+/g, ' '));
        const scopeHash = row.scope_hash || hashScope({
            scope_type: row.scope_type,
            customer_resource_name: customerResourceName,
            campaign_resource_name: row.campaign_resource_name || null,
            ad_group_resource_name: row.ad_group_resource_name || null,
            criterion_resource_name: row.criterion_resource_name || null,
            proposal_id: row.proposal_id || null,
            related_proposal_id: row.related_proposal_id || null,
            search_term_normalized: searchTermNormalized
        });

        await pool.query(
            `UPDATE semantic_memories
             SET customer_resource_name = COALESCE(customer_resource_name, $2),
                 search_term_normalized = COALESCE(search_term_normalized, $3),
                 content_hash = COALESCE(content_hash, $4),
                 scope_hash = COALESCE(scope_hash, $5)
             WHERE memory_id = $1`,
            [row.memory_id, customerResourceName, searchTermNormalized, contentHash, scopeHash]
        );
    }
}
