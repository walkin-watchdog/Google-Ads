...
export function normalizeNegativeRulesFromReports(input: NegativeReportInput): NegativeRule[] {
    ...
    for (const row of input.campaignNegatives || []) {
        ...
        rules.push({
            ...
            campaignId: String(getReportField(row, 'campaign.id') || '').trim() || null,
            campaignName: String(getReportField(row, 'campaign.name') || '').trim() || null,
            criterionId: String(getReportField(row, 'campaign_criterion.criterion_id') || '').trim() || null,
            resourceName: String(getReportField(row, 'campaign_criterion.resource_name') || '').trim() || null,
            keywordText,
            keyword: keywordText,
    ...
    for (const row of input.sharedNegativeCriteria || []) {
        ...
        const criterionId = String(getReportField(row, 'shared_criterion.criterion_id') || '').trim() || null;
        const resourceName = String(getReportField(row, 'shared_criterion.resource_name') || '').trim()
            || (input.customerId && setInfo.id && criterionId ? `customers/${input.customerId}/sharedCriteria/${setInfo.id}~${criterionId}` : null);
