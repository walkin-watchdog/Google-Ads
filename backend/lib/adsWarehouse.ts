...
export interface AudienceMetricFields extends MetricFields {
    interactions?: number | string | null;
    interaction_rate?: number | string | null;
    engagements?: number | string | null;
    engagement_rate?: number | string | null;
    active_view_impressions?: number | string | null;
    active_view_measurable_impressions?: number | string | null;
    active_view_measurable_cost_micros?: number | string | null;
    active_view_viewability?: number | string | null;
    search_impression_share?: number | string | null;
    search_rank_lost_impression_share?: number | string | null;
}

export interface AudienceDailyRow extends AudienceMetricFields {
    customer_id: string;
    date: string;
    campaign_id: string;
    ...
    status?: string | null;
    bid_modifier?: number | string | null;
    raw_payload?: Record<string, any>;
}

export interface DemographicDailyRow extends AudienceMetricFields {
    customer_id: string;
    date: string;
    campaign_id: string;
    campaign_name?: string | null;
...
export interface AudienceCriterionRow {
    customer_id: string;
    campaign_id: string;
    campaign_name?: string | null;
    ad_group_id?: string | null;
    ...
    negative?: boolean | null;
    bid_modifier?: number | string | null;
    audience_resource_name?: string | null;
...
export interface AudienceCatalogRow {
    ...
    size_for_search?: number | string | null;
    size_for_display?: number | string | null;
    membership_life_span?: number | string | null;
...
export interface KeywordClickDailyRow {
    customer_id: string;
    date: string;
...
export interface DashboardReportBundle {
    ...
    auctionInsightsPreviousRows: AuctionInsightsRow[];
    auctionInsightsPreviousPerformance: Array<{ date: string; impressions: number | string | null }>;
    auctionInsightsStatus: AuctionInsightsStatusRow[];
    candidateSignals: CandidateSignalRow[];
    coverage: CoverageEntry[];
}

export type DashboardReportBundleView = 'overview' | 'performance' | 'keywords' | 'attribution' | 'rank' | 'proposals' | 'audiences';
...
const DEFAULT_DASHBOARD_CLICK_DETAIL_ROW_LIMIT = 5_000;
...
export function emptyDashboardReportBundle(overrides: Partial<DashboardReportBundle> = {}): DashboardReportBundle {
    return {
        ...
        adGroupAudienceDaily: [],
        ageRangeDaily: [],
        genderDaily: [],
        incomeRangeDaily: [],
        parentalStatusDaily: [],
        campaignAudienceCriteria: [],
        adGroupAudienceCriteria: [],
        audienceCatalog: [],
        configuredKeywords: [],
        qualityScores: [],
        negatives: {
...
function dashboardClickDetailRowLimit(): number {
    return positiveIntegerEnv('DASHBOARD_CLICK_DETAIL_ROW_LIMIT', DEFAULT_DASHBOARD_CLICK_DETAIL_ROW_LIMIT);
}
...
CREATE TABLE IF NOT EXISTS google_ads_campaign_audience_daily (
  customer_id TEXT NOT NULL,
  date DATE NOT NULL,
  campaign_id TEXT NOT NULL,
  campaign_name TEXT,
  ...
  active_view_impressions BIGINT NOT NULL DEFAULT 0,
  active_view_measurable_impressions BIGINT NOT NULL DEFAULT 0,
  active_view_measurable_cost_micros BIGINT NOT NULL DEFAULT 0,
  active_view_viewability NUMERIC(18,8),
  search_impression_share NUMERIC(18,8),
  search_rank_lost_impression_share NUMERIC(18,8),
  run_id TEXT REFERENCES google_ads_refresh_runs(id) ON DELETE SET NULL,
  fetched_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  raw_payload JSONB NOT NULL DEFAULT '{}'::jsonb,
  PRIMARY KEY (customer_id, date, campaign_id, criterion_id)
);
CREATE INDEX IF NOT EXISTS google_ads_campaign_audience_daily_scope_idx ON google_ads_campaign_audience_daily(customer_id, campaign_id, date);

CREATE TABLE IF NOT EXISTS google_ads_ad_group_audience_daily (
  customer_id TEXT NOT NULL,
  date DATE NOT NULL,
  campaign_id TEXT NOT NULL,
  campaign_name TEXT,
...
CREATE INDEX IF NOT EXISTS google_ads_ad_group_audience_daily_scope_idx ON google_ads_ad_group_audience_daily(customer_id, campaign_id, ad_group_id, date);

CREATE TABLE IF NOT EXISTS google_ads_age_range_daily (
  customer_id TEXT NOT NULL,
  date DATE NOT NULL,
  campaign_id TEXT NOT NULL,
  campaign_name TEXT,
...
CREATE INDEX IF NOT EXISTS google_ads_age_range_daily_scope_idx ON google_ads_age_range_daily(customer_id, campaign_id, ad_group_id, date);

CREATE TABLE IF NOT EXISTS google_ads_gender_daily (LIKE google_ads_age_range_daily INCLUDING ALL);
CREATE TABLE IF NOT EXISTS google_ads_income_range_daily (LIKE google_ads_age_range_daily INCLUDING ALL);
CREATE TABLE IF NOT EXISTS google_ads_parental_status_daily (LIKE google_ads_age_range_daily INCLUDING ALL);
CREATE INDEX IF NOT EXISTS google_ads_gender_daily_scope_idx ON google_ads_gender_daily(customer_id, campaign_id, ad_group_id, date);
CREATE INDEX IF NOT EXISTS google_ads_income_range_daily_scope_idx ON google_ads_income_range_daily(customer_id, campaign_id, ad_group_id, date);
CREATE INDEX IF NOT EXISTS google_ads_parental_status_daily_scope_idx ON google_ads_parental_status_daily(customer_id, campaign_id, ad_group_id, date);
...
CREATE TABLE IF NOT EXISTS google_ads_keyword_click_daily (
  customer_id TEXT NOT NULL,
  date DATE NOT NULL,
...
CREATE INDEX IF NOT EXISTS google_ads_keyword_click_daily_scope_idx ON google_ads_keyword_click_daily(customer_id, campaign_id, ad_group_id, date);
CREATE INDEX IF NOT EXISTS google_ads_keyword_click_daily_text_idx ON google_ads_keyword_click_daily(customer_id, lower(keyword_text), date);
...
CREATE TABLE IF NOT EXISTS google_ads_campaign_snapshot (
  ...
  targeting_restrictions JSONB NOT NULL DEFAULT '[]'::jsonb,
...
CREATE TABLE IF NOT EXISTS google_ads_campaign_audience_criteria (
  customer_id TEXT NOT NULL,
  campaign_id TEXT NOT NULL,
  campaign_name TEXT,
  criterion_id TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS google_ads_campaign_audience_criteria_scope_idx
  ON google_ads_campaign_audience_criteria(customer_id, campaign_id, criterion_type, negative, present_in_latest_snapshot);

CREATE TABLE IF NOT EXISTS google_ads_ad_group_audience_criteria (
  customer_id TEXT NOT NULL,
  campaign_id TEXT NOT NULL,
  campaign_name TEXT,
  ad_group_id TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS google_ads_ad_group_audience_criteria_scope_idx
  ON google_ads_ad_group_audience_criteria(customer_id, campaign_id, ad_group_id, criterion_type, negative, present_in_latest_snapshot);

CREATE TABLE IF NOT EXISTS google_ads_user_interest_catalog (
  customer_id TEXT NOT NULL,
  ...
  size_for_search BIGINT,
  size_for_display BIGINT,
  membership_life_span BIGINT,
  read_only BOOLEAN,
...
CREATE INDEX IF NOT EXISTS google_ads_user_interest_catalog_lookup_idx
  ON google_ads_user_interest_catalog(customer_id, audience_type, category, lower(name), present_in_latest_snapshot);

CREATE TABLE IF NOT EXISTS google_ads_life_event_catalog (LIKE google_ads_user_interest_catalog INCLUDING ALL);
CREATE TABLE IF NOT EXISTS google_ads_detailed_demographic_catalog (LIKE google_ads_user_interest_catalog INCLUDING ALL);
CREATE TABLE IF NOT EXISTS google_ads_user_list_catalog (LIKE google_ads_user_interest_catalog INCLUDING ALL);
CREATE TABLE IF NOT EXISTS google_ads_custom_audience_catalog (LIKE google_ads_user_interest_catalog INCLUDING ALL);
CREATE TABLE IF NOT EXISTS google_ads_combined_audience_catalog (LIKE google_ads_user_interest_catalog INCLUDING ALL);
CREATE TABLE IF NOT EXISTS google_ads_audience_catalog (LIKE google_ads_user_interest_catalog INCLUDING ALL);
...
CREATE INDEX IF NOT EXISTS google_ads_keyword_click_daily_watermark_idx ON google_ads_keyword_click_daily(customer_id, date, fetched_at DESC);
CREATE INDEX IF NOT EXISTS google_ads_keyword_click_daily_scoped_watermark_idx ON google_ads_keyword_click_daily(customer_id, campaign_id, ad_group_id, date, fetched_at DESC);
...
CREATE INDEX IF NOT EXISTS google_ads_auction_insights_rows_dashboard_idx ON google_ads_auction_insights_rows(customer_id, source_scope, entity_id, auction_date, domain);
...
const WATERMARK_FACT_TABLES = [
    ...
    'google_ads_keyword_click_daily',
    ...
    'google_ads_income_range_daily',
    'google_ads_parental_status_daily'
];
...
const SNAPSHOT_FINGERPRINT_TABLES = [
    ...
    'google_ads_campaign_audience_criteria',
    'google_ads_ad_group_audience_criteria',
    'google_ads_user_interest_catalog',
    'google_ads_life_event_catalog',
    'google_ads_detailed_demographic_catalog',
    'google_ads_user_list_catalog',
    'google_ads_custom_audience_catalog',
    'google_ads_combined_audience_catalog',
    'google_ads_audience_catalog'
];
...
function isoDate(value: any): string | null {
    ...
    if (value instanceof Date) {
        // node-postgres parses SQL DATE values at local midnight. Converting those
        // values through UTC shifts the calendar day backward in positive-offset
        // timezones (for example Asia/Kolkata). Preserve the database calendar day.
...
const audienceMetricColumns = [
    ...
    'active_view_viewability', 'search_impression_share', 'search_rank_lost_impression_share'
];
const campaignAudienceDailyColumns = [
    'customer_id', 'date', 'campaign_id', 'campaign_name', 'criterion_id', 'criterion_resource_name',
    'criterion_type', 'negative', 'status', 'bid_modifier', ...audienceMetricColumns,
    'run_id', 'fetched_at', 'raw_payload'
];
const adGroupAudienceDailyColumns = [
    'customer_id', 'date', 'campaign_id', 'campaign_name', 'ad_group_id', 'ad_group_name', 'criterion_id',
    'criterion_resource_name', 'criterion_type', 'negative', 'status', 'bid_modifier', ...audienceMetricColumns,
    'run_id', 'fetched_at', 'raw_payload'
];
const demographicDailyColumns = [
    'customer_id', 'date', 'campaign_id', 'campaign_name', 'ad_group_id', 'ad_group_name', 'criterion_id',
    'criterion_resource_name', 'demographic_value', 'negative', 'status', 'bid_modifier', ...audienceMetricColumns,
    'run_id', 'fetched_at', 'raw_payload'
];
...
const keywordClickDailyColumns = [
    'customer_id', 'date', 'dimension_hash', 'campaign_id', 'campaign_name', 'ad_group_id', 'ad_group_name',
    'criterion_id', 'criterion_resource_name', 'keyword_text', 'match_type', 'slot', 'device', 'clicks',
    'run_id', 'fetched_at', 'raw_payload'
];
...
const conversionSearchTermDailyColumns = [
    'customer_id', 'date', 'dimension_hash', 'campaign_id', 'campaign_name', 'ad_group_id', 'ad_group_name',
    'search_term', 'matched_keyword_text', 'matched_keyword_match_type', 'conversion_action_name',
...
const campaignSnapshotColumns = [
    ...
    'campaign_budget_resource_name', 'budget_amount_micros', 'target_cpa_micros', 'target_roas',
    'advertising_channel_type', 'advertising_channel_sub_type', 'targeting_restrictions',
...
const campaignAudienceCriterionColumns = [
    'customer_id', 'campaign_id', 'campaign_name', 'criterion_id', 'criterion_resource_name', 'criterion_type',
    'status', 'negative', 'bid_modifier', 'audience_resource_name', 'audience_id', 'demographic_value',
    'present_in_latest_snapshot', 'first_seen_at', 'last_seen_at', 'snapshot_run_id', 'raw_payload'
];
const adGroupAudienceCriterionColumns = [
    'customer_id', 'campaign_id', 'campaign_name', 'ad_group_id', 'ad_group_name', 'criterion_id',
    'criterion_resource_name', 'criterion_type', 'status', 'negative', 'bid_modifier', 'audience_resource_name',
    'audience_id', 'demographic_value', 'present_in_latest_snapshot', 'first_seen_at', 'last_seen_at',
    'snapshot_run_id', 'raw_payload'
];
const audienceCatalogColumns = [
    'customer_id', 'resource_name', 'audience_id', 'name', 'audience_type', 'category', 'parent_resource_name',
    'description', 'status', 'eligible_for_search', 'eligible_for_display', 'size_for_search', 'size_for_display',
    'membership_life_span', 'read_only', 'members', 'present_in_latest_snapshot', 'first_seen_at', 'last_seen_at',
    'snapshot_run_id', 'raw_payload'
];
...
const WAREHOUSE_FINGERPRINT_COLUMNS_BY_TABLE = new Map<string, string[]>([
    ...
    ['google_ads_campaign_audience_daily', campaignAudienceDailyColumns],
    ['google_ads_ad_group_audience_daily', adGroupAudienceDailyColumns],
    ['google_ads_age_range_daily', demographicDailyColumns],
    ['google_ads_gender_daily', demographicDailyColumns],
    ['google_ads_income_range_daily', demographicDailyColumns],
    ['google_ads_parental_status_daily', demographicDailyColumns],
    ['google_ads_keyword_daily', keywordDailyColumns],
    ['google_ads_keyword_click_daily', keywordClickDailyColumns],
    ...
    ['google_ads_campaign_audience_criteria', campaignAudienceCriterionColumns],
    ['google_ads_ad_group_audience_criteria', adGroupAudienceCriterionColumns],
    ['google_ads_user_interest_catalog', audienceCatalogColumns],
    ['google_ads_life_event_catalog', audienceCatalogColumns],
    ['google_ads_detailed_demographic_catalog', audienceCatalogColumns],
    ['google_ads_user_list_catalog', audienceCatalogColumns],
    ['google_ads_custom_audience_catalog', audienceCatalogColumns],
    ['google_ads_combined_audience_catalog', audienceCatalogColumns],
    ['google_ads_audience_catalog', audienceCatalogColumns],
...
async function selectFingerprintCustomers(
    client: PoolClient,
    source: WarehouseFingerprintSource
): Promise<string[]> {
    const clauses = [`customer_id IS NOT NULL`, `btrim(customer_id) <> ''`];
    if (SNAPSHOT_FINGERPRINT_TABLES.includes(source.table)) clauses.push(`present_in_latest_snapshot = true`);
    const { rows } = await client.query(
    ...
    const customerIds = rows
        .map(row => cleanText(row.customer_id))
        .filter((customerId): customerId is string => Boolean(customerId));
    return Array.from(new Set(customerIds)).sort();
}

async function selectAllFingerprintRowsForSource(
    client: PoolClient,
    source: WarehouseFingerprintSource,
    customerId: string
): Promise<Record<string, any>[]> {
    return selectFingerprintCustomerRows(
        client,
        source.table,
        customerId,
        SNAPSHOT_FINGERPRINT_TABLES.includes(source.table)
    );
}

async function rebuildWarehouseSourceFingerprints(
    client: PoolClient,
    source: WarehouseFingerprintSource,
    customerId: string
): Promise<void> {
    await client.query(`LOCK TABLE ${source.table} IN SHARE MODE`);
    const rows = await selectAllFingerprintRowsForSource(client, source, customerId);
    await client.query(
        `DELETE FROM google_ads_warehouse_slice_fingerprints
         WHERE customer_id = $1 AND source_table = $2`,
        [customerId, source.table]
    );
    await upsertWarehouseFingerprintRows(client, buildSliceFingerprintRows(source, [], rows, customerId));
}

export async function rebuildWarehouseSliceFingerprints(db: Db, customerId?: string): Promise<void> {
    await withTransaction(db, async client => {
        if (customerId) {
            await client.query(`DELETE FROM google_ads_warehouse_slice_fingerprints WHERE customer_id = $1`, [customerId]);
            for (const source of WAREHOUSE_FINGERPRINT_SOURCES) {
                await rebuildWarehouseSourceFingerprints(client, source, customerId);
            }
        } else {
            await client.query(`DELETE FROM google_ads_warehouse_slice_fingerprints`);
            for (const source of WAREHOUSE_FINGERPRINT_SOURCES) {
                const customerIds = await selectFingerprintCustomers(client, source);
                for (const rowCustomerId of customerIds) {
                    await rebuildWarehouseSourceFingerprints(client, source, rowCustomerId);
                }
            }
        }
    });
...
export async function upsertWarehouseRefreshRunStarted(pool: Pool, input: {
    id: string;
    kind: WarehouseRefreshKind;
    customerId?: string | null;
    requestedStartDate?: string | null;
    ...
    await pool.query(
        `INSERT INTO google_ads_refresh_runs
         ...
         ON CONFLICT (id) DO UPDATE SET
             customer_id = COALESCE(EXCLUDED.customer_id, google_ads_refresh_runs.customer_id),
             kind = EXCLUDED.kind,
             status = 'running',
             requested_start_date = COALESCE(EXCLUDED.requested_start_date, google_ads_refresh_runs.requested_start_date),
             requested_end_date = COALESCE(EXCLUDED.requested_end_date, google_ads_refresh_runs.requested_end_date),
...
export const replaceKeywordClickDailyWindow = (db: Db, customerId: string, startDate: string, endDate: string, rows: KeywordClickDailyRow[], runId: string) =>
    replaceDateWindow(db, 'google_ads_keyword_click_daily', keywordClickDailyColumns, customerId, startDate, endDate, rows, runId);
...
export type PreparedMappedReportReplacement = {
    customerId: string;
    reportName: string;
    storagePlans: MappedRowsStoragePlan[];
    datePlans: Array<{ table: string; startDate: string; endDate: string; previousRows: WarehouseFingerprintRow[] }>;
    snapshotPlans: Array<{ table: string; previousRows: WarehouseFingerprintRow[] }>;
    client: PoolClient | null;
    ownsClient: boolean;
    completed: boolean;
};

const MAPPED_REPORT_STORAGE_PLANS = new Map<string, MappedRowsStoragePlan[]>([
    ['account_summary', [{ field: 'accountDaily', table: 'google_ads_account_daily', columns: accountDailyColumns, mode: 'date' }]],
    ['campaign_performance', [{ field: 'campaignDaily', table: 'google_ads_campaign_daily', columns: campaignDailyColumns, mode: 'date' }]],
    ['ad_group_performance', [{ field: 'adGroupDaily', table: 'google_ads_ad_group_daily', columns: adGroupDailyColumns, mode: 'date' }]],
    ['campaign_audience_performance', [{ field: 'campaignAudienceDaily', table: 'google_ads_campaign_audience_daily', columns: campaignAudienceDailyColumns, mode: 'date' }]],
    ['ad_group_audience_performance', [{ field: 'adGroupAudienceDaily', table: 'google_ads_ad_group_audience_daily', columns: adGroupAudienceDailyColumns, mode: 'date' }]],
    ['age_range_performance', [{ field: 'ageRangeDaily', table: 'google_ads_age_range_daily', columns: demographicDailyColumns, mode: 'date' }]],
    ['gender_performance', [{ field: 'genderDaily', table: 'google_ads_gender_daily', columns: demographicDailyColumns, mode: 'date' }]],
    ['income_range_performance', [{ field: 'incomeRangeDaily', table: 'google_ads_income_range_daily', columns: demographicDailyColumns, mode: 'date' }]],
    ['parental_status_performance', [{ field: 'parentalStatusDaily', table: 'google_ads_parental_status_daily', columns: demographicDailyColumns, mode: 'date' }]],
    ['keyword_performance', [{ field: 'keywordDaily', table: 'google_ads_keyword_daily', columns: keywordDailyColumns, mode: 'date' }]],
    ['keyword_click_details', [{ field: 'keywordClickDaily', table: 'google_ads_keyword_click_daily', columns: keywordClickDailyColumns, mode: 'date' }]],
    ['search_term_performance', [{ field: 'searchTermDaily', table: 'google_ads_search_term_daily', columns: searchTermDailyColumns, mode: 'date' }]],
    ['device_performance', [{ field: 'deviceDaily', table: 'google_ads_device_daily', columns: deviceDailyColumns, mode: 'date' }]],
    ['day_of_week_performance', [{ field: 'dayOfWeekDaily', table: 'google_ads_day_of_week_daily', columns: dayOfWeekDailyColumns, mode: 'date' }]],
    ['day_and_hour_performance', [{ field: 'dayHourDaily', table: 'google_ads_day_hour_daily', columns: dayHourDailyColumns, mode: 'date' }]],
    ['landing_page_performance', [{ field: 'landingPageDaily', table: 'google_ads_landing_page_daily', columns: landingPageDailyColumns, mode: 'date' }]],
    ['expanded_landing_page_performance', [{ field: 'expandedLandingPageDaily', table: 'google_ads_expanded_landing_page_daily', columns: expandedLandingPageDailyColumns, mode: 'date' }]],
    ['conversion_action_performance', [{ field: 'conversionActionDaily', table: 'google_ads_conversion_action_daily', columns: conversionActionDailyColumns, mode: 'date' }]],
    ['conversion_action_metrics_by_ad_group', [{ field: 'conversionAdGroupDaily', table: 'google_ads_conversion_ad_group_daily', columns: conversionScopedDailyColumns, mode: 'date' }]],
    ['conversion_attribution_by_search_term', [{ field: 'conversionSearchTermDaily', table: 'google_ads_conversion_search_term_daily', columns: conversionSearchTermDailyColumns, mode: 'date' }]],
    ['campaign_config', [{ field: 'campaignSnapshot', table: 'google_ads_campaign_snapshot', columns: campaignSnapshotColumns, mode: 'snapshot', conflictColumns: ['customer_id', 'campaign_id'] }]],
    ['ad_group_config', [{ field: 'adGroupSnapshot', table: 'google_ads_ad_group_snapshot', columns: adGroupSnapshotColumns, mode: 'snapshot', conflictColumns: ['customer_id', 'campaign_id', 'ad_group_id'] }]],
    ['configured_keywords', [{ field: 'configuredKeywords', table: 'google_ads_configured_keywords', columns: configuredKeywordColumns, mode: 'snapshot', conflictColumns: ['customer_id', 'campaign_id', 'ad_group_id', 'criterion_id'] }]],
    ['quality_score', [{ field: 'qualityScores', table: 'google_ads_quality_score_snapshot', columns: qualityScoreColumns, mode: 'snapshot', conflictColumns: ['customer_id', 'campaign_id', 'ad_group_id', 'criterion_id'] }]],
    ['campaign_negatives', [{ field: 'campaignNegatives', table: 'google_ads_campaign_negatives', columns: campaignNegativeColumns, mode: 'snapshot', conflictColumns: ['customer_id', 'campaign_id', 'criterion_id'] }]],
    ['ad_group_negatives', [{ field: 'adGroupNegatives', table: 'google_ads_ad_group_negatives', columns: adGroupNegativeColumns, mode: 'snapshot', conflictColumns: ['customer_id', 'campaign_id', 'ad_group_id', 'criterion_id'] }]],
    ['account_negatives', [{ field: 'accountNegativeLists', table: 'google_ads_account_negative_lists', columns: accountNegativeListColumns, mode: 'snapshot', conflictColumns: ['customer_id', 'customer_negative_criterion_id'] }]],
    ['shared_negative_sets', [{ field: 'sharedNegativeSets', table: 'google_ads_shared_negative_sets', columns: sharedNegativeSetColumns, mode: 'snapshot', conflictColumns: ['customer_id', 'shared_set_id'] }]],
    ['shared_negative_criteria', [{ field: 'sharedNegativeCriteria', table: 'google_ads_shared_negative_criteria', columns: sharedNegativeCriterionColumns, mode: 'snapshot', conflictColumns: ['customer_id', 'shared_set_resource_name', 'criterion_id'] }]],
    ['campaign_shared_sets', [{ field: 'campaignSharedSets', table: 'google_ads_campaign_shared_sets', columns: campaignSharedSetColumns, mode: 'snapshot', conflictColumns: ['customer_id', 'campaign_id', 'shared_set_resource_name'] }]],
    ['campaign_audience_criteria', [{ field: 'campaignAudienceCriteria', table: 'google_ads_campaign_audience_criteria', columns: campaignAudienceCriterionColumns, mode: 'snapshot', conflictColumns: ['customer_id', 'campaign_id', 'criterion_id'] }]],
    ['ad_group_audience_criteria', [{ field: 'adGroupAudienceCriteria', table: 'google_ads_ad_group_audience_criteria', columns: adGroupAudienceCriterionColumns, mode: 'snapshot', conflictColumns: ['customer_id', 'campaign_id', 'ad_group_id', 'criterion_id'] }]],
    ['user_interest_catalog', [{ field: 'audienceCatalog', table: 'google_ads_user_interest_catalog', columns: audienceCatalogColumns, mode: 'snapshot', conflictColumns: ['customer_id', 'resource_name'] }]],
    ['life_event_catalog', [{ field: 'audienceCatalog', table: 'google_ads_life_event_catalog', columns: audienceCatalogColumns, mode: 'snapshot', conflictColumns: ['customer_id', 'resource_name'] }]],
    ['detailed_demographic_catalog', [{ field: 'audienceCatalog', table: 'google_ads_detailed_demographic_catalog', columns: audienceCatalogColumns, mode: 'snapshot', conflictColumns: ['customer_id', 'resource_name'] }]],
    ['user_list_catalog', [{ field: 'audienceCatalog', table: 'google_ads_user_list_catalog', columns: audienceCatalogColumns, mode: 'snapshot', conflictColumns: ['customer_id', 'resource_name'] }]],
    ['custom_audience_catalog', [{ field: 'audienceCatalog', table: 'google_ads_custom_audience_catalog', columns: audienceCatalogColumns, mode: 'snapshot', conflictColumns: ['customer_id', 'resource_name'] }]],
    ['combined_audience_catalog', [{ field: 'audienceCatalog', table: 'google_ads_combined_audience_catalog', columns: audienceCatalogColumns, mode: 'snapshot', conflictColumns: ['customer_id', 'resource_name'] }]],
    ['audience_catalog', [{ field: 'audienceCatalog', table: 'google_ads_audience_catalog', columns: audienceCatalogColumns, mode: 'snapshot', conflictColumns: ['customer_id', 'resource_name'] }]]
]);

function storagePlansForReport(reportName: string): MappedRowsStoragePlan[] {
    return MAPPED_REPORT_STORAGE_PLANS.get(reportName.replace(/-/g, '_')) || [];
}

export async function prepareMappedReportReplacement(
    db: Db,
    customerId: string,
    reportName: string,
    startDate: string | null,
    endDate: string | null
): Promise<PreparedMappedReportReplacement> {
    const plans = storagePlansForReport(reportName);
    const state: PreparedMappedReportReplacement = {
        customerId,
        reportName,
        storagePlans: plans,
        datePlans: [],
    ...
    if (!plans.length) return state;
    const ownsClient = isPool(db);
    const client = ownsClient ? await db.connect() : db;
    state.client = client;
    state.ownsClient = ownsClient;
    try {
        await client.query('BEGIN');
        for (const plan of plans) {
            if (plan.mode === 'date') {
                if (!startDate || !endDate) throw new Error(`${reportName} requires startDate and endDate for warehouse replacement.`);
                assertDate(startDate, 'startDate');
                assertDate(endDate, 'endDate');
                const previousRows = await selectFingerprintDateRows(client, plan.table, customerId, startDate, endDate);
                await client.query(`DELETE FROM ${plan.table} WHERE customer_id = $1 AND date BETWEEN $2::date AND $3::date`, [customerId, startDate, endDate]);
                state.datePlans.push({ table: plan.table, startDate, endDate, previousRows: previousRows as WarehouseFingerprintRow[] });
            } else {
                const previousRows = await selectFingerprintCustomerRows(client, plan.table, customerId, true);
                await client.query(`UPDATE ${plan.table} SET present_in_latest_snapshot = false WHERE customer_id = $1`, [customerId]);
                state.snapshotPlans.push({ table: plan.table, previousRows: previousRows as WarehouseFingerprintRow[] });
            }
        }
    } catch (err) {
...
async function appendMappedRowsWithPlans(client: PoolClient, mapped: Record<string, any>, runId: string, plans: MappedRowsStoragePlan[]): Promise<number> {
    let appended = 0;
    for (const plan of plans) {
        const rows = Array.isArray(mapped?.[plan.field]) ? mapped[plan.field] : [];
        if (!rows.length) continue;
        if (plan.mode === 'date') {
            appended += await insertRows(client, plan.table, plan.columns, rows, runId);
        } else {
            const now = new Date();
            ...
            appended += await upsertRows(client, plan.table, plan.columns, plan.conflictColumns || ['customer_id'], enriched, runId, ['first_seen_at']);
        }
    }
    return appended;
}

export async function appendMappedReportRows(
    db: Db,
    mapped: Record<string, any>,
    runId: string,
    replacement?: PreparedMappedReportReplacement
): Promise<number> {
    if (replacement) {
        if (replacement.completed) throw new Error(`Replacement for ${replacement.reportName} is already completed.`);
        if (!replacement.client || !replacement.storagePlans.length) return 0;
        return appendMappedRowsWithPlans(replacement.client, mapped, runId, replacement.storagePlans);
    }
    const allPlans = Array.from(MAPPED_REPORT_STORAGE_PLANS.values()).flat();
    let appended = 0;
    await withTransaction(db, async client => {
        appended += await appendMappedRowsWithPlans(client, mapped, runId, allPlans);
    });
    return appended;
}

export async function finalizeMappedReportReplacement(db: Db, state: PreparedMappedReportReplacement): Promise<void> {
    if (state.completed) return;
    if (!state.client || !state.storagePlans.length) {
        state.completed = true;
        return;
    }
    try {
        const client = state.client;
        for (const plan of state.datePlans) {
            await refreshDateWindowFingerprints(client, plan.table, state.customerId, plan.startDate, plan.endDate, plan.previousRows);
        }
        for (const plan of state.snapshotPlans) {
            await refreshCustomerFingerprints(client, plan.table, state.customerId, plan.previousRows, true);
        }
        await client.query('COMMIT');
        state.completed = true;
        invalidateAdsWarehouseRuntimeCaches();
    } catch (err) {
        await abortMappedReportReplacement(state).catch(() => undefined);
        throw err;
    } finally {
        if (state.completed && state.ownsClient && state.client) {
...
export async function abortMappedReportReplacement(state: PreparedMappedReportReplacement): Promise<void> {
    if (state.completed) return;
    try {
        if (state.client && state.storagePlans.length) await state.client.query('ROLLBACK');
    } finally {
        state.completed = true;
        if (state.ownsClient && state.client) {
            state.client.release();
            state.client = null;
        }
    }
...
export async function acquireWarehouseRefreshLock(db: Pool, customerId: string): Promise<{ acquired: boolean; release: () => Promise<void> }> {
    const client = await db.connect();
    const lockName = `google_ads_refresh:${customerId}`;
    try {
        const { rows } = await client.query(`SELECT pg_try_advisory_lock(hashtext($1)) AS acquired`, [lockName]);
...
async function selectKeywordClickRows(pool: Pool, filters: DashboardFilters, limit = 0): Promise<KeywordClickDailyRow[]> {
    return selectGroupedRows<KeywordClickDailyRow>(
        pool,
        'google_ads_keyword_click_daily',
        filters,
        `
            src.customer_id,
            src.date::text AS date,
            src.dimension_hash,
            src.campaign_id,
            MAX(src.campaign_name) AS campaign_name,
            src.ad_group_id,
            ...
            MAX(src.keyword_text) AS keyword_text,
            MAX(src.match_type) AS match_type,
            COALESCE(src.slot, '') AS slot,
            COALESCE(src.device, '') AS device,
            SUM(src.clicks)::bigint AS clicks
        `,
        `src.customer_id, src.date, src.dimension_hash, src.campaign_id, src.ad_group_id, src.criterion_id, COALESCE(src.slot, ''), COALESCE(src.device, '')`,
        'src.date DESC, SUM(src.clicks) DESC, MAX(src.keyword_text) ASC',
        limit
    );
}
...
async function selectAudienceCatalogRows(pool: Pool, filters: DashboardFilters): Promise<AudienceCatalogRow[]> {
    const groups = await Promise.all(AUDIENCE_CATALOG_TABLES.map(table =>
        selectSnapshotRows<AudienceCatalogRow>(
            pool,
            table,
            { ...filters, campaignId: null, adGroupId: null },
            'audience_type ASC, category ASC NULLS LAST, name ASC'
        )
    ));
    return groups.flat().sort((left, right) =>
        String(left.audience_type).localeCompare(String(right.audience_type))
...
export async function getCoverageForWindow(pool: Pool, filters: DashboardFilters, reportNames: readonly string[] = FACT_REPORTS): Promise<CoverageEntry[]> {
    ...
    const { rows } = await pool.query(
        `SELECT report_name, coverage_date::text AS coverage_date, status, row_count, fetched_at, error
...
export async function getDashboardReportBundle(pool: Pool, filters: DashboardFilters): Promise<DashboardReportBundle> {
    ...
    const auctionComparisonFilters = previousAuctionInsightsFilters(filters);
    const [
        accountDaily,
        campaignDaily,
        adGroupDaily,
        ...
        selectKeywordClickRows(pool, filters, dashboardClickDetailRowLimit()),
        ...
        selectRows<AudienceDailyRow>(pool, 'google_ads_campaign_audience_daily', filters),
        selectRows<AudienceDailyRow>(pool, 'google_ads_ad_group_audience_daily', filters),
        selectRows<DemographicDailyRow>(pool, 'google_ads_age_range_daily', filters),
        selectRows<DemographicDailyRow>(pool, 'google_ads_gender_daily', filters),
        selectRows<DemographicDailyRow>(pool, 'google_ads_income_range_daily', filters),
        selectRows<DemographicDailyRow>(pool, 'google_ads_parental_status_daily', filters),
        selectSnapshotRows<AudienceCriterionRow>(pool, 'google_ads_campaign_audience_criteria', filters, 'campaign_name ASC NULLS LAST, criterion_type ASC, criterion_id ASC'),
        selectSnapshotRows<AudienceCriterionRow>(pool, 'google_ads_ad_group_audience_criteria', filters, 'campaign_name ASC NULLS LAST, ad_group_name ASC NULLS LAST, criterion_type ASC, criterion_id ASC'),
        selectAudienceCatalogRows(pool, filters),
        ...
        auctionComparisonFilters ? selectAuctionRows(pool, auctionComparisonFilters) : Promise.resolve([]),
        auctionComparisonFilters ? selectAuctionScopePerformance(pool, auctionComparisonFilters) : Promise.resolve([]),
...
export async function getDashboardReportBundleForView(
    ...
    const [
        ...
        selectAuctionRows(pool, filters),
        (() => {
            const comparisonFilters = previousAuctionInsightsFilters(filters);
            return comparisonFilters ? selectAuctionRows(pool, comparisonFilters) : Promise.resolve([]);
        })(),
        (() => {
            const comparisonFilters = previousAuctionInsightsFilters(filters);
            return comparisonFilters ? selectAuctionScopePerformance(pool, comparisonFilters) : Promise.resolve([]);
        })(),
        selectCustomerRows<AuctionInsightsStatusRow>(pool, 'google_ads_auction_insights_status', filters.customerId, 'entity_type ASC, entity_name ASC NULLS LAST'),
        selectCandidateSignals(pool, filters, dashboardCandidateSignalRowLimit())
    ]);
...
function previousAuctionInsightsFilters(filters: DashboardFilters): DashboardFilters | null {
    const selectedDates = dateRange(filters.startDate, filters.endDate);
    // Google surfaces period-over-period entrants and exits for recent windows.
    // Avoid an expensive and misleading comparison for long/all-time selections.
    if (selectedDates.length > 31) return null;
    const previousEnd = new Date(`${filters.startDate}T00:00:00.000Z`);
    previousEnd.setUTCDate(previousEnd.getUTCDate() - 1);
    const previousStart = new Date(previousEnd);
    previousStart.setUTCDate(previousStart.getUTCDate() - selectedDates.length + 1);
    return {
        ...filters,
...
async function selectAuctionRows(pool: Pool, filters: DashboardFilters): Promise<AuctionInsightsRow[]> {
    const clauses = [
        'customer_id = $1',
        'auction_date BETWEEN $2::date AND $3::date'
    ];
    const params: any[] = [filters.customerId, filters.startDate, filters.endDate];
    if (filters.adGroupId) {
        params.push(filters.adGroupId);
    ...
    const { rows } = await pool.query(
        `SELECT *
         ...
         WHERE ${clauses.join(' AND ')}
         ORDER BY auction_date ASC, domain ASC`,
        params
    );
    return rows.map(normalizeDbRow) as AuctionInsightsRow[];
}

async function selectAuctionScopePerformance(
    pool: Pool,
    filters: DashboardFilters
): Promise<Array<{ date: string; impressions: number | string | null }>> {
    const params: any[] = [filters.customerId, filters.startDate, filters.endDate];
    let table = 'google_ads_account_daily';
    let scopeClause = '';
    if (filters.adGroupId) {
        table = 'google_ads_ad_group_daily';
        params.push(filters.adGroupId);
        scopeClause = ` AND ad_group_id = $${params.length}`;
    } else if (filters.campaignId) {
        table = 'google_ads_campaign_daily';
        params.push(filters.campaignId);
        scopeClause = ` AND campaign_id = $${params.length}`;
    }
    const { rows } = await pool.query(
        `SELECT date, SUM(impressions)::numeric AS impressions
         FROM ${table}
         WHERE customer_id = $1
           AND date BETWEEN $2::date AND $3::date${scopeClause}
         GROUP BY date
         ORDER BY date ASC`,
        params
    );
    return rows.map(normalizeDbRow) as Array<{ date: string; impressions: number | string | null }>;
}
...
function warehouseFingerprintStrictScopeSql(
    source: WarehouseFingerprintSource,
    filters: DashboardFilters,
    param: (value: any) => string
): string {
    if (filters.adGroupId && tableSupportsAdGroup(source.table)) {
        const clauses = [`scope_level = 'ad_group'`, `ad_group_id = ${param(filters.adGroupId)}`];
        if (filters.campaignId) clauses.push(`campaign_id = ${param(filters.campaignId)}`);
        return `(${clauses.join(' AND ')})`;
    }
    if (filters.campaignId && tableSupportsCampaign(source.table)) {
        return `(scope_level = 'campaign' AND campaign_id = ${param(filters.campaignId)})`;
    }
    return `(scope_level = 'account')`;
}

function warehouseFingerprintParentAwareScopeSql(
    source: WarehouseFingerprintSource,
    filters: DashboardFilters,
    param: (value: any) => string
): string {
    if (filters.adGroupId && tableSupportsAdGroup(source.table)) {
        const parentClauses = [`scope_level = 'account_parent'`];
        if (filters.campaignId) {
            parentClauses.push(`(scope_level = 'campaign_parent' AND campaign_id = ${param(filters.campaignId)})`);
        } else {
            parentClauses.push(`scope_level = 'campaign_parent'`);
        }
        const childClauses = [`scope_level = 'ad_group'`, `ad_group_id = ${param(filters.adGroupId)}`];
        if (filters.campaignId) childClauses.push(`campaign_id = ${param(filters.campaignId)}`);
        parentClauses.push(`(${childClauses.join(' AND ')})`);
        return `(${parentClauses.join(' OR ')})`;
    }
    if (filters.campaignId && tableSupportsCampaign(source.table)) {
        return `(scope_level = 'account_parent' OR (scope_level = 'campaign' AND campaign_id = ${param(filters.campaignId)}))`;
    }
    return `(scope_level = 'account')`;
}

function warehouseFingerprintScopeSql(filters: DashboardFilters, param: (value: any) => string): string {
    return `(${WAREHOUSE_FINGERPRINT_SOURCES.map(source => {
        const sourceParam = param(source.table);
        const scopeSql = source.parentAware
            ? warehouseFingerprintParentAwareScopeSql(source, filters, param)
            : warehouseFingerprintStrictScopeSql(source, filters, param);
        return `(source_table = ${sourceParam} AND ${scopeSql})`;
    }).join(' OR ')})`;
}

async function computeWarehouseWatermark(pool: Pool, filters: DashboardFilters): Promise<string> {
    const params: any[] = [filters.customerId, FINGERPRINT_GLOBAL_SLICE, filters.startDate, filters.endDate];
    const param = (value: any): string => {
        params.push(value);
        return `$${params.length}`;
    };
    const scopeSql = warehouseFingerprintScopeSql(filters, param);
    const { rows } = await pool.query(
...
export async function getCachedDashboardPayload(pool: Pool, rawFilters: DashboardFilters, watermark: string): Promise<any | null> {
    ...
    const { rows } = await pool.query(
        `SELECT payload, payload_bytes
         FROM dashboard_payload_cache
         WHERE cache_key = $1
           AND warehouse_watermark = $2
           AND ($3::integer = 0 OR payload_bytes <= $3::integer)`,
