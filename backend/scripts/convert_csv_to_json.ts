...
const defaultOutputPath = path.resolve(__dirname, '..', 'data', 'latest', 'auction-insights-domains.json');
...
if (!inputPath) {
    console.error('Pass an explicit Auction Insights CSV path: bun run scripts/convert_csv_to_json.ts /path/to/export.csv');
    process.exit(1);
}
