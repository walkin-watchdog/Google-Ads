...
const targetCPA = summary.cpa || 1857;
...
const targetCPAs = {
  competitor: catStats.competitor.conv > 0 ? safeDiv(catStats.competitor.spend, catStats.competitor.conv) : (summary.cpa || 1857),
  generic: catStats.generic.conv > 0 ? safeDiv(catStats.generic.spend, catStats.generic.conv) : (summary.cpa || 1857)
};
...
const keywordData = (Array.isArray(keywords) ? keywords : [keywords]).map(k => {
  ...
  const catTargetCPA = targetCPAs[cat as keyof typeof targetCPAs] || targetCPAs.generic;
  ...
  else if (hasPrimaryConv && cpa > 0 && cpa < catTargetCPA) label = '🚀 Scale candidate';
...
const conversionActions = (Array.isArray(conversionActionsRaw) ? conversionActionsRaw : [conversionActionsRaw]).filter(Boolean).flatMap(a => {
  ...
  const metricsForAction = conversionMetrics.filter(m => m['segments.conversion_action_name'] === actionName);
...
const auctionInsights = (Array.isArray(auctionInsightsRaw) ? auctionInsightsRaw : [auctionInsightsRaw]).filter(Boolean).map(a => ({
  campaign: a['campaign.name'] || null,
  domain: a['segments.auction_insight_domain'],
...
if (trendData.length >= 7) {
  ...
  trendData.forEach(d => {
    ...
    const catTargetCPA = targetCPAs[cat as keyof typeof targetCPAs] || targetCPAs.generic;
    if (d.cpa > 0 && d.cpa > catTargetCPA * 2) {
      anomalies.push({ date: d.date, metric: 'cpa', value: d.cpa, threshold: catTargetCPA * 2, message: `CPA spike: ₹${d.cpa} (2× ${cat} target)` });
    }
  });
}
