...
import { getLeadAttributionSummary } from '../lib/leads';
import {
...
import { plannerFields } from '../lib/plannerScoring';
import { buildSourceCoverage as buildSourceCoverageFromReports, type SourceCoverageReport } from '../lib/sourceCoverage';
...
const accountNegativesRaw = readJSON(path.join(LATEST, 'account-negatives.json')) || [];
const sharedNegativeSetsRaw = readJSON(path.join(LATEST, 'shared-negative-sets.json')) || [];
const sharedNegativeCriteriaRaw = readJSON(path.join(LATEST, 'shared-negative-criteria.json')) || [];
const campaignSharedSetsRaw = readJSON(path.join(LATEST, 'campaign-shared-sets.json')) || [];
...
const SOURCE_COVERAGE_REPORTS: SourceCoverageReport[] = [
  { name: 'configured_keywords', fileName: 'configured-keywords.json' },
  { name: 'keyword_performance', fileName: 'keyword-performance.json' },
  { name: 'search_term_performance', fileName: 'search-term-performance.json' },
  { name: 'campaign_negatives', fileName: 'campaign-negatives.json' },
  { name: 'ad_group_negatives', fileName: 'ad-group-negatives.json' },
  { name: 'account_negatives', fileName: 'account-negatives.json' },
  { name: 'shared_negative_sets', fileName: 'shared-negative-sets.json' },
  { name: 'shared_negative_criteria', fileName: 'shared-negative-criteria.json' },
  { name: 'campaign_shared_sets', fileName: 'campaign-shared-sets.json' },
  { name: 'keyword_planner_ideas', fileName: 'keyword-planner-ideas.json' },
  { name: 'keyword_planner_historical_metrics', fileName: 'keyword-planner-historical-metrics.json' },
  { name: 'auction_insights_domains', fileName: 'auction-insights-domains.json' },
  { name: 'auction_insights_status', fileName: 'auction-insights-status.json' },
  { name: 'quality_score', fileName: 'quality-score.json' },
  { name: 'landing_page_performance', fileName: 'landing-page-performance.json' },
  { name: 'expanded_landing_page_performance', fileName: 'expanded-landing-page-performance.json' },
  { name: 'device_performance', fileName: 'device-performance.json' },
  { name: 'day_of_week_performance', fileName: 'day-of-week-performance.json' },
  { name: 'day_and_hour_performance', fileName: 'day-and-hour-performance.json' }
];

function buildSourceCoverage(): SourceCoverageSummary {
  return buildSourceCoverageFromReports(LATEST, SOURCE_COVERAGE_REPORTS);
}
...
const currency = currencyCodeFromAccountRow(acct, 'INR');
const accountId = String(acct['customer.id'] || process.env.GOOGLE_ADS_CUSTOMER_ID || '6780466013');
const fallbackCpaBenchmark = cpaBenchmarkForAccount(summary.cpa, currency);
...
const COMPETITORS = COMPETITOR_ROOTS;
...
const configuredKeywordRules: ConfiguredKeywordRule[] = (Array.isArray(configuredKeywordsRaw) ? configuredKeywordsRaw : [configuredKeywordsRaw])
  .filter(Boolean)
  .map(configuredKeywordRuleFromReportRow)
  .filter((row): row is ConfiguredKeywordRule => Boolean(row));

// Normalize all negative keyword sources into one canonical rule table.
const negativesData: NegativeRule[] = normalizeNegativeRulesFromReports({
  customerId: accountId,
  accountNegatives: Array.isArray(accountNegativesRaw) ? accountNegativesRaw : [accountNegativesRaw].filter(Boolean),
  campaignNegatives: Array.isArray(campaignNegativesRaw) ? campaignNegativesRaw : [campaignNegativesRaw].filter(Boolean),
  adGroupNegatives: Array.isArray(adGroupNegativesRaw) ? adGroupNegativesRaw : [adGroupNegativesRaw].filter(Boolean),
  sharedNegativeSets: Array.isArray(sharedNegativeSetsRaw) ? sharedNegativeSetsRaw : [sharedNegativeSetsRaw].filter(Boolean),
  sharedNegativeCriteria: Array.isArray(sharedNegativeCriteriaRaw) ? sharedNegativeCriteriaRaw : [sharedNegativeCriteriaRaw].filter(Boolean),
  campaignSharedSets: Array.isArray(campaignSharedSetsRaw) ? campaignSharedSetsRaw : [campaignSharedSetsRaw].filter(Boolean)
});

function termScope(campaignId: any, campaignName: any, adGroupId?: any, adGroupName?: any): TermScope {
  return {
    customerId: accountId,
...
function decisionFields(term: string, scope: TermScope, allowAnyScope = false): Record<string, any> {
  return flattenDecisionContext(decisionContextForTerm(term, scope, negativesData, configuredKeywordRules, { allowAnyScope }));
}

for (const row of keywordData) {
  Object.assign(row, decisionFields(row.keyword, termScope(row.campaignId, row.campaign, row.adGroupId, row.adGroup)));
}
...
const searchTermData = (Array.isArray(searchTerms) ? searchTerms : [searchTerms])
  ...
  .map(s => {
    ...
    const coverage = decisionFields(term, termScope(s['campaign.id'], s['campaign.name'], s['ad_group.id'], s['ad_group.name']));
    if (coverage.isNegativeCovered) label = 'Already excluded';
    else if (coverage.isConfiguredKeyword) label = 'Already configured';
    return {
...
const existingKeywordSet = new Set(configuredKeywordRules.map(row => normKey(row.keywordText || row.keyword)));
...
const keywordPlannerIdeas = (Array.isArray(keywordPlannerIdeasRaw) ? keywordPlannerIdeasRaw : [keywordPlannerIdeasRaw])
  .filter(Boolean)
  .map(row => {
    const coverage = decisionFields(row.keyword, { customerId: accountId }, true);
    return {
      ...row,
      ...plannerFields(row.keyword, row, {}, fallbackCpaBenchmark),
      inAccountKeyword: coverage.isConfiguredKeyword || existingKeywordSet.has(normKey(row.keyword)),
      ...
      plannerClassification: coverage.isNegativeCovered
        ? 'blocked_by_negative'
        : coverage.isConfiguredKeyword || existingKeywordSet.has(normKey(row.keyword))
          ? 'already_configured'
          : existingSearchTermSet.has(normKey(row.keyword))
            ? 'already_seen'
            : 'new_opportunity',
      ...coverage
    };
...
const competitorBreakdown = COMPETITOR_ROOTS.map(name => {
  ...
  const searchRows: any[] = (searchTermData as any[]).filter(row => normKey(row.searchTerm).includes(name));
  const negativeCoverageKnown = searchRows.length > 0;
  const impressions = sum(rows, 'impressions');
  const clicks = sum(rows, 'clicks');
  const spend = +sum(rows, 'spend').toFixed(2);
  const conversions = +sum(rows, 'conversions').toFixed(2);
  const searchSpend = +sum(searchRows, 'spend').toFixed(2);
  const searchConversions = +sum(searchRows, 'conversions').toFixed(2);
  const negativeCoveredSpend = +sum(searchRows.filter(row => row.isNegativeCovered), 'spend').toFixed(2);
  const negativeUncoveredSpend = negativeCoverageKnown ? +(Math.max(searchSpend - negativeCoveredSpend, 0)).toFixed(2) : null;
  const negativeCoveredClicks = sum(searchRows.filter(row => row.isNegativeCovered), 'clicks');
  const negativeUncoveredClicks = sum(searchRows.filter(row => !row.isNegativeCovered), 'clicks');
  const coverageSources = Array.from(new Set(searchRows
    .filter(row => row.isNegativeCovered)
    .map(row => row.negativeCoverageSource)
    .filter(Boolean)));
  ...
  return {
    ...
    negativeCoveredClicks,
    negativeUncoveredClicks,
    negativeCoverageSources: coverageSources,
    coverageMismatchSpend: negativeCoverageKnown ? negativeCoveredSpend : null,
  };
});
...
function enrichCandidateSignal(signal: any): any {
  ...
  const context = decisionContextForTerm(String(term), scope, negativesData, configuredKeywordRules, { allowAnyScope: !scope.campaignId });
  return {
    ...signal,
    decisionContext: context,
    coverage: {
      ...context.negativeCoverage,
      ...context.configuredKeywordCoverage
    }
  };
}

const candidateSignals = (Array.isArray(candidateSignalsRaw) ? candidateSignalsRaw : []).map(enrichCandidateSignal);
const sourceCoverage = buildSourceCoverage();

function decisionInputs(leadAttribution: any = null): Record<string, any> {
  return {
    leadAttributionStatus: leadAttribution ? 'ok' : 'missing',
    leadAttributionRows: leadAttribution?.totals?.uniqueLeads || 0,
    leadAttributionGeneratedAt: leadAttribution?.generatedAt || null,
    keywordPlannerStatus: keywordPlanner.status?.status || 'empty',
    auctionInsightsRows: auctionInsights.length,
    auctionInsightsStatusRows: auctionInsightsStatus.length,
    qualityScoreRows: qualityScoreData.length,
...
function rebuildPayloadDecisionContext(payload: any, leadAttribution: any = null): void {
  payload.decisionContext = buildDecisionContextSummary({
    negativeRules: negativesData,
    configuredKeywords: configuredKeywordRules,
    searchTerms: payload.searchTerms,
    plannerIdeas: payload.keywordPlanner?.ideas || [],
    plannerHistoricalMetrics: payload.keywordPlanner?.historicalMetrics || [],
    candidateSignals: payload.candidateSignals || [],
    sourceCoverage,
    decisionInputs: decisionInputs(leadAttribution)
  });
}
...
rebuildPayloadDecisionContext(payload, null);
...
async function savePayload(payload: any) {
    ...
    if (process.env.DATABASE_URL) {
        try {
            ...
            try {
                const leadAttribution = await getLeadAttributionSummary(pool, payload);
                enrichDashboardDecisionRows(payload, leadAttribution);
                rebuildPayloadDecisionContext(payload, leadAttribution);
            } catch (e: any) {
                console.warn(`Could not attach first-party lead attribution: ${e?.message || e}`);
                enrichDashboardDecisionRows(payload, null);
                rebuildPayloadDecisionContext(payload, null);
            }
            ...
            const existingLeadAttribution = payload.leadAttribution || null;
            enrichDashboardDecisionRows(payload, existingLeadAttribution);
            rebuildPayloadDecisionContext(payload, existingLeadAttribution);
        } finally {
