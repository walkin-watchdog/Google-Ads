...
const SOURCE_STATUS_FILE = path.join(DATA_DIR, 'source-status.json');
...
async function run() {
    ...
    try {
        ...
        writeSourceStatus(sourceSummary);
        if (pool) {
