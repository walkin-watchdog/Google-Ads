...
function buildCurrentState(keywordData: any[], campaignData: any[], searchTermData: any[]) {
    ...
    for (const row of keywordData) {
        ...
        const obj = { campaignId, adGroupId, criterionId, text, matchType, status: row.adGroupCriterion?.status, bidMicros: row.adGroupCriterion?.cpcBidMicros == null ? null : Number(row.adGroupCriterion.cpcBidMicros), biddingStrategy: row.campaign?.biddingStrategyType || campaigns[campaignId]?.biddingStrategy || 'UNKNOWN' };
...
async function run() {
    ...
    const state = buildCurrentState(keywordData, campaignData, searchTermData);
    ...
    for (const row of rows) {
        ...
        const baseline = row.baseline_metrics || option.baseline_metrics || campaignBaseline(campaignData, result.campaignId);
        await pool.query(
