...
describe('semantic memory schema', () => {
    test('initialization creates the final schema without repair migrations', async () => {
        let sql = '';
        const pool = {
            async query(query) {
                sql += `\n${query}`;
        ...
        expect(sql).toContain('CREATE TABLE IF NOT EXISTS semantic_memories');
        expect(sql).toContain('memory_id UUID PRIMARY KEY DEFAULT gen_random_uuid()');
        expect(sql).toContain('scope_hash VARCHAR(64) NOT NULL');
        expect(sql).toContain('embedding VECTOR(1536) NOT NULL');
        expect(sql).toContain('semantic_memory_embeddings_1536_embedding_hnsw_cosine_idx');
        expect(sql).toContain('USING hnsw (embedding vector_cosine_ops)');
        expect(sql).not.toContain('ALTER TABLE semantic_memories ADD COLUMN');
        expect(sql).not.toContain('ALTER TABLE semantic_memory_embeddings_1536 ADD COLUMN');
        expect(sql).not.toContain('UPDATE semantic_memory_embeddings_1536 e');
        expect(sql).not.toContain("conrelid = 'semantic_memories'::regclass");
    });
});
