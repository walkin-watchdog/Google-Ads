...
describe('dashboard payload warehouse slices', () => {
    ...
    test('attribution view loads full lead events required by journey paths', async () => {
        clearDashboardViewPayloadCache();
        clearAdsWarehouseRuntimeCaches();
        const pool = new RecordingPool();
        const filters = {
        ...
        const payload = await buildDashboardPayloadForView(pool, filters, 'attribution', {
            filtersResolved: true
        });

        expect(payload.meta.payloadView).toBe('attribution');
        expect(pool.queries.some(query =>
            query.includes('SELECT session_key, session_key_type')
            && query.includes('FROM lead_sessions')
            && query.includes('ORDER BY last_seen DESC')
        )).toBe(true);
        expect(pool.queries.some(query =>
            query.includes('FROM lead_sessions')
