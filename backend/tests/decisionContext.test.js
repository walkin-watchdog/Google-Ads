...
describe('normalizeNegativeRulesFromReports', () => {
    ...
    test('does not read invalid v24 customer_negative_criterion keyword fields', () => {
        const rules = normalizeNegativeRulesFromReports({
            customerId: '123',
            accountNegatives: [{
                'customer.id': '123',
                'customer_negative_criterion.type': 'NEGATIVE_KEYWORD_LIST',
                'customer_negative_criterion.negative_keyword_list.shared_set': 'customers/123/sharedSets/9',
                'customer_negative_criterion.keyword.text': 'invalid removed field',
                'customer_negative_criterion.keyword.match_type': 'BROAD',
                'customer_negative_criterion.status': 'ENABLED'
            }]
        });

        expect(rules).toEqual([]);
        expect(decisionContextForTerm('invalid removed field query', {}, rules, []).negativeCoverage.isNegativeCovered).toBe(false);
    });
});
