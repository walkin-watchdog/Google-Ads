...
describe('dashboard PWA/auth/push plan coverage', () => {
    test('initializes named users before auth sessions and push tables', () => {
        const schema = fs.readFileSync(path.join(root, 'lib', 'schema.ts'), 'utf8');
        expect(schema.indexOf('ensureDashboardUsersSchema(pool)')).toBeLessThan(schema.indexOf('ensureDashboardAuthSchema(pool)'));
        expect(schema.indexOf('ensureDashboardAuthSchema(pool)')).toBeLessThan(schema.indexOf('ensureDashboardPushSchema(pool)'));
    });
    ...
    test('browser account controls use blocker-neutral routes', () => {
        const app = fs.readFileSync(path.join(root, 'client', 'app.js'), 'utf8');
        const server = fs.readFileSync(path.join(root, 'server.ts'), 'utf8');
        ...
        expect(server).not.toContain("'/api/google-ads/mutations/preview'");
