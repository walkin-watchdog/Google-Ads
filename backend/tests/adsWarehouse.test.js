...
class FakeWarehouseDb {
    ...
    async query(sql, params = []) {
        ...
        if (compact === 'BEGIN') {
            this.transactionSnapshots.push(JSON.parse(JSON.stringify(this.tables)));
        ...
        if (compact.startsWith('SELECT pg_advisory_lock') || compact.startsWith('SELECT pg_advisory_unlock')) return { rows: [{}], rowCount: 1 };
        ...
        if (compact === 'DELETE FROM google_ads_warehouse_slice_fingerprints WHERE customer_id = $1 AND source_table = $2') {
            const before = this.tables.google_ads_warehouse_slice_fingerprints.length;
            this.tables.google_ads_warehouse_slice_fingerprints = this.tables.google_ads_warehouse_slice_fingerprints
                .filter(row => row.customer_id !== params[0] || row.source_table !== params[1]);
            return { rows: [], rowCount: before - this.tables.google_ads_warehouse_slice_fingerprints.length };
        }
    ...
    selectRows(table, sql, params) {
        ...
        return rows
            .filter(row => {
                ...
                if (sql.includes("status = 'succeeded'") && row.status !== 'succeeded') return false;
                if (/\bcampaign_id = \$4(?!\d)/.test(sql) && String(row.campaign_id) !== String(params[3])) return false;
                if (/\bad_group_id = \$5(?!\d)/.test(sql) && String(row.ad_group_id) !== String(params[4])) return false;
...
describe('ads warehouse repository', () => {
    test('streamed report replacement rolls back old rows when aborted', async () => {
        const db = new FakeWarehouseDb();
        db.tables.google_ads_campaign_daily = [{
        ...
        const replacement = await prepareMappedReportReplacement(db, '1234567890', 'campaign_performance', '2026-06-25', '2026-06-25');
        await appendMappedReportRows(db, {
            campaignDaily: [{
                customer_id: '1234567890',
                date: '2026-06-25',
                campaign_id: 'new',
        ...
        expect(db.tables.google_ads_campaign_daily.map(row => row.campaign_id)).toEqual(['new']);
        await abortMappedReportReplacement(replacement);
        expect(db.tables.google_ads_campaign_daily).toEqual([{
            customer_id: '1234567890',
            date: '2026-06-25',
            campaign_id: 'old',
            campaign_name: 'Old campaign',
    ...
    test('streamed conversion search-term rows include search_term in warehouse inserts', async () => {
        const db = new FakeWarehouseDb();
        const mapped = mapReportRows('conversion_attribution_by_search_term', '1234567890', [{
            ...
            'segments.keyword.info.text': 'whatsapp crm tool',
            ...
            'metrics.conversions_value': 50
        }]);

        await appendMappedReportRows(db, mapped, 'run1');

        const insertSql = db.queries.find(query => query.startsWith('INSERT INTO google_ads_conversion_search_term_daily'));
        expect(insertSql).toContain('search_term');
        expect(insertSql).toContain('matched_keyword_text');
        expect(db.tables.google_ads_conversion_search_term_daily).toHaveLength(1);
        expect(db.tables.google_ads_conversion_search_term_daily[0]).toMatchObject({
            customer_id: '1234567890',
            search_term: 'whatsapp crm',
            matched_keyword_text: 'whatsapp crm tool',
            matched_keyword_match_type: 'PHRASE',
            conversion_action_name: 'Submit lead form'
        });
    });
    ...
    test('warehouse schema includes maintained fingerprint storage for dashboard watermarks', async () => {
        ...
        expect(schemaSql).not.toContain('SELECT EXISTS (SELECT 1 FROM google_ads_warehouse_slice_fingerprints');
        expect(schemaSql).not.toContain('google_ads_warehouse_fingerprint_backfills');
        expect(schemaSql).not.toContain('ALTER TABLE');
    });

    test('schema initialization does not rebuild fingerprints from existing warehouse rows', async () => {
        const db = new FakeWarehouseDb();
        db.tables.google_ads_campaign_daily.push({
        ...
        await ensureAdsWarehouseSchema(db);

        expect(db.queries.some(query => query.startsWith('SELECT pg_advisory_lock'))).toBe(false);
        expect(db.queries.some(query => query.includes('google_ads_warehouse_fingerprint_backfills'))).toBe(false);
        expect(db.queries.some(query => query === 'LOCK TABLE google_ads_campaign_daily IN SHARE MODE')).toBe(false);
        expect(db.queries.some(query =>
            query === 'DELETE FROM google_ads_warehouse_slice_fingerprints WHERE customer_id = $1 AND source_table = $2'
        )).toBe(false);
    });
    ...
    test('reads DB dashboard cache only through stored payload byte sizes', async () => {
        const previousLimit = process.env.DASHBOARD_DB_CACHE_MAX_BYTES;
        process.env.DASHBOARD_DB_CACHE_MAX_BYTES = '64';
        const queries = [];
        const db = {
        ...
        try {
            ...
            expect(queries[0].sql).toContain('payload_bytes <= $3');
            expect(queries[0].sql).not.toContain('octet_length');
            expect(queries.some(query => query.sql.startsWith('DELETE FROM dashboard_payload_cache'))).toBe(false);
        } finally {
            if (previousLimit === undefined) delete process.env.DASHBOARD_DB_CACHE_MAX_BYTES;
            else process.env.DASHBOARD_DB_CACHE_MAX_BYTES = previousLimit;
        }
