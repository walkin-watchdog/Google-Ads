...
class FakeLeadPool {
    ...
    async query(sql, params = []) {
        ...
        if (compact.includes('COUNT(*) FILTER')) {
            return { rows: [{
                needs_review: 1,
                qualified_or_converted: 2,
                ready_rows: 1,
                rushed_clickthrough: 4,
                inconsistent_answers: 3,
                weak_business_detail: 2,
                high_abuse_path: 1,
                challenge_pending: 2,
                challenge_passed: 5,
                challenge_failed: 1
...
class FakeOfflineExportPool {
    ...
    async query(sql, params = []) {
        ...
        const statuses = Array.isArray(params[0]) ? params[0] : [];
        ...
        return {
            rows: rows.filter(row =>
                ...
                (!campaignValues.length || campaignValues.includes(row.attribution.utm_campaign))
            )
        };
    }
}
...
class FakeFullLeadPool {
    async query(sql) {
        const compact = sql.replace(/\s+/g, ' ').trim();
        if (compact.includes('CREATE TABLE IF NOT EXISTS lead_events')) return { rows: [] };
        if (compact.includes('SELECT session_key, session_key_type') && compact.includes('FROM lead_sessions')) {
            return {
                rows: ['111', '222'].map((campaignId, index) => ({
                    session_key: `session_id:${campaignId}`,
                    session_key_type: 'session_id',
                    status: index === 0 ? 'qualified' : 'useless',
                    status_rank: index === 0 ? 1 : 4,
                    event_count: index === 0 ? 3 : 2,
                    lead_ids: [`lead-${campaignId}`],
                    attribution: { utm_campaign: campaignId, utm_term: 'same search term', keyword: 'same keyword', match_type: 'EXACT' },
                    contact: {},
        ...
        if (compact.includes('FROM lead_events')) {
            return {
                rows: [
                    {
                        session_key: 'session_id:111',
                        kind: 'demo',
                        tracking_only: true,
                    ...
                    },
                    {
                        ...
                        kind: 'whatsapp',
                        tracking_only: false,
                        submitted_at: '2026-01-10T00:01:00.000Z',
                        received_at: '2026-01-10T00:01:00.000Z'
                    },
                    {
                        session_key: 'session_id:111',
                        kind: 'signup',
                        tracking_only: false,
                        submitted_at: '2026-01-10T00:02:00.000Z',
                        received_at: '2026-01-10T00:02:00.000Z'
                    },
                    {
                        session_key: 'session_id:222',
                        kind: 'demo',
                        tracking_only: true,
                        submitted_at: '2026-01-10T00:00:00.000Z',
                        received_at: '2026-01-10T00:00:00.000Z'
...
describe('lead attribution overview summary', () => {
    test('uses lead_sessions aggregates without loading lead_events', async () => {
        ...
        expect(summary.recentLeads[0].leadSource).toBe('whatsapp_widget');
        expect(summary.recentLeads[0].leadSourceKind).toBe('whatsapp_widget');
        expect(summary.safeguards).toEqual({
            rushedClickthrough: 4,
            inconsistentAnswers: 3,
            weakBusinessDetail: 2,
            highAbusePath: 1,
            pending: 2,
            passed: 5,
            failed: 1
        ...
        expect(pool.queries.some(query =>
            !query.includes('CREATE TABLE IF NOT EXISTS lead_events') &&
            query.includes('FROM lead_events')
        )).toBe(false);
    });
...
describe('full lead attribution summary', () => {
    test('keeps identical search terms scoped to their campaigns', async () => {
        const summary = await getLeadAttributionSummary(new FakeFullLeadPool(), dashboardData);

        expect(summary.bySearchTerm).toHaveLength(2);
        expect(summary.safeguards).toEqual({
        ...
        expect(summary.bySearchTerm.map(row => row.campaignId).sort()).toEqual(['111', '222']);
        expect(summary.bySearchTerm.find(row => row.campaignId === '111')).toMatchObject({ qualified: 1, useless: 0 });
        expect(summary.bySearchTerm.find(row => row.campaignId === '222')).toMatchObject({ qualified: 0, useless: 1 });
        expect(summary.allLeads.find(row => row.sessionKey === 'session_id:111')).toMatchObject({
            actionPath: 'whatsapp -> signup',
            actionCount: 2,
            eventCount: 2
        });
        expect(summary.allLeads.find(row => row.sessionKey === 'session_id:222')).toMatchObject({
            actionPath: 'demo',
            actionCount: 1,
            eventCount: 1
        });
...
describe('offline conversion export', () => {
    test('matches selected campaign by id or known campaign name', async () => {
        ...
        expect(pool.queries.some(query => query.sql.includes("typedChallenge'->>'status"))).toBe(true);
    ...
    test('never exports Maybe as an offline conversion', async () => {
        const pool = new FakeOfflineExportPool();

        const result = await exportOfflineConversionsCsv(pool, { statuses: 'maybe' });

        expect(result.statuses).toEqual([]);
        expect(result.rowCount).toBe(0);
        expect(pool.queries.some(query => Array.isArray(query.params[0]) && query.params[0].length === 0)).toBe(true);
    });
});
