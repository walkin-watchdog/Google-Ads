...
describe('frontend server-filtering contract', () => {
    test('dashboard assets satisfy the browser parser and content security policy', () => {
        expect(appJs).toContain('<path d="M22 21v-2a4 4 0 0 0-3-3.87"></path>');
        expect(appJs).not.toContain('<path d="M22 21v-2a4 4 0 0 3-3.87"></path>');
        expect(serverTs).toContain("font-src 'self' data:");
    });
    ...
    test('dashboard API defaults an omitted view to the full payload', () => {
        expect(serverTs).toContain("view || 'full'");
        expect(serverTs).not.toContain('Dashboard view is required.');
    });
