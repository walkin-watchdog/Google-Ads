...
describe('dashboard view warehouse bundles', () => {
    ...
    test('overview view reads small segment summaries for visible overview charts only', async () => {
        ...
        expect(pool.queried('google_ads_keyword_click_daily')).toBe(true);
        ...
        expect(pool.queryFor('google_ads_keyword_click_daily')).toContain('GROUP BY');
        expect(pool.queryFor('google_ads_keyword_click_daily')).toContain('LIMIT');
    ...
    test('rank view reads grouped bounded rank-support rows without configured keywords', async () => {
        ...
        const auctionSql = pool.queryFor('google_ads_auction_insights_rows');
        expect(auctionSql).toContain('auction_date BETWEEN $2::date AND $3::date');
        expect(auctionSql).toContain("source_scope = 'account'");
        expect(auctionSql).not.toContain('LIMIT');
        expect(pool.queryFor('candidate_signals')).toContain('LIMIT');
    });

    test('rank Auction Insights uses one exact hierarchy scope', async () => {
        const campaignPool = new RecordingPool();
        await getDashboardReportBundleForView(campaignPool, {
            ...filters,
            campaignId: '111'
        }, 'rank');
        const campaignSql = campaignPool.queryFor('google_ads_auction_insights_rows');
        expect(campaignSql).toContain("source_scope = 'campaign'");
        expect(campaignSql).not.toContain("source_scope = 'account'");

        const adGroupPool = new RecordingPool();
        await getDashboardReportBundleForView(adGroupPool, {
            ...filters,
            campaignId: '111',
            adGroupId: '222'
        }, 'rank');
        const adGroupSql = adGroupPool.queryFor('google_ads_auction_insights_rows');
        expect(adGroupSql).toContain("source_scope = 'ad_group'");
        expect(adGroupSql).not.toContain("source_scope = 'campaign'");
    });

    test('SQL DATE values keep their database calendar day outside UTC', async () => {
        const pool = new RecordingPool();
        pool.query = async function (sql, params = []) {
            const compact = sql.replace(/\s+/g, ' ').trim();
            this.queries.push(compact);
            if (compact.includes('FROM google_ads_auction_insights_rows') && params[1] === filters.startDate) {
                return {
                    rows: [{
                        customer_id: filters.customerId,
                        dimension_hash: 'calendar-date',
                        source_scope: 'account',
                        entity_id: filters.customerId,
                        auction_date: new Date(2026, 0, 15),
        ...
        const bundle = await getDashboardReportBundleForView(pool, filters, 'rank');

        expect(bundle.auctionInsightsRows[0].auction_date).toBe('2026-01-15');
    });
    ...
    test('audiences view reads audience, demographic, criteria, and catalog data without unrelated keyword tables', async () => {
        const pool = new RecordingPool();

        await getDashboardReportBundleForView(pool, filters, 'audiences');
        ...
        ]) expect(pool.queried(table)).toBe(true);
        expect(pool.queried('google_ads_keyword_daily')).toBe(false);
        expect(pool.queried('google_ads_search_term_daily')).toBe(false);
        expect(pool.queried('google_ads_keyword_planner_ideas')).toBe(false);
        expect(pool.queried('google_ads_auction_insights_rows')).toBe(false);
    ...
    test('warehouse watermarks read maintained slice fingerprints for filtered views', async () => {
        ...
        expect(watermarkSql).toContain('source_table =');
        expect(watermarkSql).toContain("scope_level = 'ad_group'");
        expect(watermarkSql).toContain("scope_level = 'campaign_parent'");
        expect(watermarkSql).toContain("scope_level = 'account_parent'");
        expect(watermarkSql).toContain('ad_group_id =');
        expect(watermarkSql).toContain('campaign_id =');
    });
