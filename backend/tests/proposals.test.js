...
describe('proposal memory context', () => {
    test('normalizes optional proposal and option memory_context', () => {
        ...
        const withoutMemoryContext = normalizeProposal({
            proposal_id: 'prop_without_memory_context',
            type: 'DIAGNOSE',
            summary: 'Proposal without memory context',
            options: [{
                option_id: 'diagnose',
                strategy_type: 'DIAGNOSE',
                hypothesis: 'No memory context exists.',
                recommendation: 'Render the proposal without memory context.',
                verification_spec: diagnosisVerification()
            }]
        });
        expect(withoutMemoryContext.memory_context).toBeNull();
        expect(withoutMemoryContext.options[0].memory_context).toBeNull();
    });
});
