...
describe('Zenseeo qualification progress webhook normalization', () => {
    ...
    test('payloads without safeguards keep paid-path inference', () => {
        const event = normalizeLeadWebhookPayload(basePayload({
            status: 'in_progress',
            trigger: 'answer',
            revision: 5,
    ...
    test('requires explicit tracking-only metadata', () => {
        const implicit = normalizeLeadWebhookPayload(basePayload({}, {
            event_id: 'qualification_progress:lead-progress-1:4'
        }));
        const explicit = normalizeLeadWebhookPayload(basePayload({}, {
            event_id: 'qualification_progress:lead-progress-1:5',
            tracking_only: true
        }));

        expect(implicit.tracking_only).toBe(false);
        expect(explicit.tracking_only).toBe(true);
    });
