...
describe('frontend user preference synchronization', () => {
    ...
    test('loads named-user browser preferences only from user-scoped storage', () => {
        const userId = '11111111-2222-4333-8444-555555555555';
        const userMetrics = ['conversions', 'cpa', 'cvr', 'allConversions', 'spend', 'impressions', 'clicks'];
        const storage = new MemoryStorage({ [userKey(userId, METRICS_KEY)]: JSON.stringify(userMetrics) });
        const harness = createPreferenceHarness({
            storage,
            dashboardFetch: async () => new Response('{}'),
            auth: { user: { id: userId } }
        ...
        harness.reload();
        expect(harness.metrics()).toEqual(userMetrics);
    });
