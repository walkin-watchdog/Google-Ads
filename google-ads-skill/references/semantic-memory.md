...
Memory tools are MCP workflow tools. They require an initialized session, successful `confirm_google_ads_skill`, and the appropriate scoped MCP key; search is read-scoped, while create/store/deactivate/link operations are proposal-scoped. Treat policy/rate-limit failures as MCP access issues, not as absence of memory.
...
Before creating proposals, call `search_memories` when campaign, ad-group, keyword, search-term, proposal, or account-level human context could matter and a valid externally generated 1536-dimension query embedding is available. If no embedding provider or valid embedding is available, do not send placeholder, dummy, hand-built, zero, partial, or truncated vectors. Skip the memory search, explicitly name that limitation in the proposal caveats or response, and continue with current account evidence.
...
Use the same external embedding model and `dimensions: 1536` used for stored memory embeddings. Do not attempt to satisfy the `query_embedding` schema with a manually constructed fallback vector. Treat returned memories as context, not proof. If memory changes proposal framing, ranking, or risk, add proposal or option `memory_context` with:
