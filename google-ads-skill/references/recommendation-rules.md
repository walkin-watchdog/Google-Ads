...
Do not call this machine learning. Historical success rates are only high-confidence priors from past observed impact-tracking outcomes.
...
Before any proposal or mutation tool call in a new MCP session, ensure `confirm_google_ads_skill` has succeeded. A policy error for missing skill confirmation, missing scope, or rate limit is not evidence about the account; fix the MCP/session issue before interpreting data.
...
## Mutation Safety Loop

Recommendations and proposals do not execute account changes. If the user explicitly asks to make a supported change, use the mutation path:

1. Re-check the relevant evidence and current configured state.
2. Preview with the relevant `google_ads_preview_*` tool or entity-specific browser control action.
3. Show the diff, warnings, operation count, confirmation token, and expiry.
4. Ask for explicit confirmation.
5. Execute only with `google_ads_confirm_mutation`.

Supported changes are positive keyword add/remove, campaign/ad-group negative keyword add/remove, exact-scope audience changes, campaign ad schedule add/remove/replace, and campaign/ad-group pause/resume. Audience changes cover supported campaign/ad-group segment additions/removals, exclusions, demographic exclusions, bid modifiers, explicit Targeting/Observation settings, and separately reviewed custom-segment creation. Preserve the exact campaign/ad-group owner; custom-segment creation and attachment require separate reviewed operations. A positive-keyword add may include one explicit user/evidence-backed HTTP(S) `finalUrl` of at most 2,048 characters, which must appear in the preview diff; never infer it or attach it to negatives/removals. Do not attempt account-level/shared-list negative creation, campaign/ad-group removal, budget/bid/ad edits, or direct offline conversion uploads in this workflow.

Mutation preview and confirm are scoped/rate-limited MCP tools. Confirmation tokens are redacted in MCP audit logs. If live confirmation fails, do not blindly retry; check mutation history and the returned error/request ID, because live mutate execution is intentionally not retried by the Google Ads client.

## User Decision Loop

Proposal decisions only record intent:
