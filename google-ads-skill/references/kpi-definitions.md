...
| Screening Leads | Deduped lead sessions with `status = "maybe"` |
...
| In-Progress Leads | `maybe + qualified` |
...
First-party lead quality is stronger than raw Google Ads conversion count when present. `leadAttribution` separates repeated conversion events from deduped people and reports `new`, `useless`, `maybe`, `qualified`, `qualified_lost`, and `converted` statuses. `maybe` is screening, `maybe + qualified` are in progress, and `qualified_lost`, `converted`, and `useless` are terminal. `qualifiedPipeline` means `qualified + qualified_lost + converted`; `qualifiedOrConverted` means `qualified + converted`. State clearly whether a CPA is Google Ads CPA, True CPA, Qualified CPA, Converted CPA, or Customer CPA.
