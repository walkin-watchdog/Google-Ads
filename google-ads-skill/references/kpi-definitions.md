...
First-party lead quality is stronger than raw Google Ads conversion count when present. `leadAttribution` separates repeated conversion events from deduped people and reports `useless`, `qualified`, `qualified_lost`, and `converted` statuses. `qualified` is in progress; `qualified_lost`, `converted`, and `useless` are terminal. State clearly whether a CPA is Google Ads CPA, True CPA, Qualified CPA, or Customer CPA.
